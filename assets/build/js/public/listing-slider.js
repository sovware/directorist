...
(function ($) {
  ...
  function allListingSlider() {
    ...
    singleListingSlider.forEach(function (el, i) {
      ...
      var dataWidthUnit = el.getAttribute('data-width-unit');
      var dataHeight = el.getAttribute('data-height');
      var dataPosition = el.getAttribute('data-position');
      var dataColumns = parseInt(el.getAttribute('data-columns'), 10);
      var dataGap = parseInt(el.getAttribute('data-gap'), 10);
      ...
      var sliderWidthUnit = dataWidthUnit === 'percentage' ? '%' : 'px';
      var sliderWidth = dataWidth ? dataWidth + sliderWidthUnit : '100%';
      var applySliderPosition = function applySliderPosition(slider) {
        if (!slider) {
          return;
        }
        var positions = {
        ...
        var margins = positions[dataPosition] || positions.center;
      ...
      var singleSliderColumns = Math.min(3, Math.max(1, dataColumns || 1));
      var singleSliderGap = Math.min(100, Math.max(0, Number.isNaN(dataGap) ? 20 : dataGap));
      var desktopSliderColumns = Math.min(singleSliderColumns, singleSliderTotalSlides.length || 1);
      var tabletSliderColumns = Math.min(2, desktopSliderColumns);
      var singleSliderLoopEnable = singleSliderTotalSlides.length > desktopSliderColumns;
      ...
      var swiperSingleListingConfig = {
        ...
        breakpoints: {
          768: {
            ...
            spaceBetween: tabletSliderColumns > 1 ? singleSliderGap : 0
          },
          1200: {
            slidesPerView: desktopSliderColumns,
            spaceBetween: desktopSliderColumns > 1 ? singleSliderGap : 0
          }
        }
      };
