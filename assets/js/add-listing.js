				...
				function getFieldValue(fieldKey, $) {
					...
					if (
						...
						if ($imageUploadWrapper.length) {
							...
							var $previewSection = $imageUploadWrapper.find(
				...
				function evaluateArrayCondition(
					...
					switch (operator) {
						...
						case '=':
							...
							var condValStrForIs = String(condVal)
								.toLowerCase()
								.trim();

							// Normalize all array values to strings for comparison
							var normalizedValues = fieldArray.map(
								function (val) {
									if (typeof val === 'string') {
										...
										if (val.value)
											return String(val.value)
												...
												.toLowerCase();
										if (val.id)
											return String(val.id).toLowerCase();
										return String(val).toLowerCase();
									}
									return String(val).toLowerCase();
								}
							...
							var hasMatch = normalizedValues.some(
								function (val) {
							...
							if (taxonomyFieldSelector) {
								...
								setTimeout(function () {
									...
									if ($taxField.length) {
										...
										// Try Select2 API
										if (
											typeof $taxField.select2 ===
											'function'
										) {
											try {
												var selectedData =
													$taxField.select2('data');
												if (
													selectedData &&
													selectedData.length > 0
												) {
						...
						mutations.forEach(function (mutation) {
							...
							if (
								...
								if (
									...
									var $imageWrapper = $target.closest(
										'.directorist-form-image-upload-field'
									);
								...
								if (
									...
									var _$imageWrapper = $target.closest(
										'.directorist-form-image-upload-field'
									);
								...
								mutation.addedNodes.forEach(function (node) {
									if (node.nodeType === 1) {
										...
										if (
											...
											var _$imageWrapper2 = $node.closest(
												'.directorist-form-image-upload-field'
											);
								...
								mutation.removedNodes.forEach(function (node) {
									if (node.nodeType === 1) {
										...
										if (
											...
											$node.closest('.ez-media-uploader')
												.length ||
											$node.hasClass(
