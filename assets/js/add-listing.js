...
$(function () {
  ...
  $('body').on('click', '#addNewSocial', function (e) {
    e.preventDefault();
    var $container = $('#social_info_sortable_container');
    var template = $('#tmpl-directorist-listing-form-social-item');
    if (template.length) {
      var socialFieldsCount = $container.find('.directorist-form-social-fields').length;
      var html = template.html().replace(/_ID_/g, socialFieldsCount);
      $container.append(html);
    }
  });
  function addFirstSocialItem() {
    if (!$('#social_info_sortable_container').find('.directorist-form-social-fields').length) {
      $('#addNewSocial').trigger('click');
    }
  }
  addFirstSocialItem();
