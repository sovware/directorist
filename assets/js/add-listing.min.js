	...
	!(function () {
		...
		function d() {
			...
			([
				...
				f(c(c({}, { taxonomy: 'tags' }), e)));
		}
		function l(t) {
			f(c(c({}, { taxonomy: 'categories' }), t));
		}
		function u(t) {
			f(c(c({}, { taxonomy: 'locations' }), t));
		}
		function f(t) {
			if (
				(t = c(c({}, { selector: '', url: '', taxonomy: 'tags' }), t))
					.selector
			) {
				var e = s(t.selector),
				...
				if ('tags' !== t.taxonomy) {
					var a = e.closest('.directorist-search-form'),
						...
						l = i.find('input[name="directory_type"]'),
						u = null;
					(a.length &&
						(u = a.find(
				...
				var f = 1,
					p = {
						...
						templateResult: function (t) {
							...
							!e &&
								t.element &&
								(e = s(t.element).attr('data-icon') || '');
							var i = 0;
							if (void 0 !== t.level && null !== t.level)
								i = parseInt(t.level);
							else if (t.element) {
								var r = s(t.element).attr('data-level');
								if (null != r) i = parseInt(r);
								else {
									var n = t.text.match(/^\s+/),
										o = n ? n[0].length : 0;
									o > 0 && (i = Math.floor(o / 8));
								}
							}
							var a = t.text.trim(),
								c =
							...
							return (
								i > 0 &&
									(d.addClass('item-level-' + (i + 1)),
									d.css('padding-left', 20 * i + 'px')),
								d.html(c),
								d
							);
						},
				...
				if (
					...
					m(e, e.data('selected-id'), e.data('selected-label'));
				}
			}
		}
		function p() {
		...
		(window.addEventListener('load', d),
			...
			i(5386));
		var _ = jQuery,
			v = directorist.add_listing_data;
		function g() {
			return v.is_admin ? '#post' : '#directorist-add-listing-form';
		}
		function w() {
			var t = _('.directorist-color-field-js', g());
			...
			if (
				...
				function c(i) {
					(e.forEach(function (t, e) {
						...
						i > 1 &&
							_('.multistep-wizard__btn--prev').removeAttr(
								'disabled'
							));
				...
				(r && (r.innerHTML = o),
					...
					_('.multistep-wizard__btn--prev').on('click', function (t) {
						(t.preventDefault(),
							o > 1 &&
								(c(--o),
								o <= 1 && this.setAttribute('disabled', !0)));
					}),
					_('.multistep-wizard__btn--next').on('click', function (t) {
						(t.preventDefault(), o < e.length && c(++o));
					}),
					_('.multistep-wizard__nav__btn').on('click', function (t) {
						if (
							(t.preventDefault(),
						...
						o <= 1 &&
							_('.multistep-wizard__btn--prev').attr(
								'disabled',
								!0
							);
					}));
		...
		(_(window).on('load', function () {
			(_('input#manual_coordinate').length &&
				_('input#manual_coordinate').each(function (t, e) {
					_(e).is(':checked') ||
						(_('#hide_if_no_manual_cor').hide(),
						_('.directorist-map-coordinates').hide());
				}),
				w());
		}),
			_(function () {
				function t(t) {
					return _(
						'#'.concat(_('[for="'.concat(t, '"]')).data('option'))
					);
				}
				if (
					(_('body').on('click', '#manual_coordinate', function (t) {
						_('input#manual_coordinate').is(':checked')
							? (_('.directorist-map-coordinates').show(),
								_('#hide_if_no_manual_cor').show())
							: (_('.directorist-map-coordinates').hide(),
								_('#hide_if_no_manual_cor').hide());
					}),
					_('body').on('click', '#addNewSocial', function (t) {
						var e = this,
							i = _('#social_info_sortable_container'),
							r = _('.directorist-form-social-fields').length,
							n = 'id='.concat(r),
							o = jQuery('#addNewSocial');
						(_('.directorist-form-social-fields').each(
							function (t, e) {
								var i = _(e);
					...
					_('body').on(
						...
						'.directorist-form-social-fields__remove',
						function (t) {
							var e = _(this).data('id'),
								i = _('div#socialID-'.concat(e));
							swal(
								{
									title: v.i18n_text.confirmation_text,
									text: v.i18n_text.ask_conf_sl_lnk_del_txt,
									type: 'warning',
									showCancelButton: !0,
									confirmButtonColor: '#DD6B55',
									confirmButtonText:
										v.i18n_text.confirm_delete,
									closeOnConfirm: !1,
								},
								function (t) {
									t &&
					...
					_('.directorist-form-pricing-field__options').on(
						'change',
						'input',
						function () {
							var e = _(this)
					...
					_('.directorist-form-pricing-field').hasClass(
						'price-type-both'
					))
				) {
					_('#price_range, #price').hide();
					var i = _(
				...
				var n = _('#has_tagline').val();
				(_('#has_excerpt').val() && n
					? _('.atbd_tagline_moto_field').fadeIn()
					: _('.atbd_tagline_moto_field').fadeOut(),
					_('#atbd_optional_field_check').on('change', function () {
						_(this).is(':checked')
							? _('.atbd_tagline_moto_field').fadeIn()
							: _('.atbd_tagline_moto_field').fadeOut();
					}),
					_('#listing_t_c').on('click', function (t) {
						(t.preventDefault(),
							_('#tc_container').toggleClass('active'));
					}));
				var o = (function (t) {
					if ('' == t) return {};
				...
				function a() {
					if (
						...
						r(
							null == v
								? void 0
								: v.category_custom_field_relations
						)
					) {
						var t = [],
							i = 0,
							n = v.category_custom_field_relations,
						...
						if (
							...
							for (
								...
								var l = e(d[s], 2),
									...
									h = p.closest('.directorist-form-group');
								t.has(f)
				...
				(window.addEventListener('load', function () {
					...
					_('#at_biz_dir-categories').on('change', function () {
						a();
					}),
					_('#at_biz_dir-categorychecklist').on(
						'change',
						function () {
							a();
						}
					...
					directorist.is_admin ||
						(directorist.lazy_load_taxonomy_fields
							? _('#at_biz_dir-categories').on(
									'select2:select',
									function () {
										_('#at_biz_dir-categories').trigger(
											'change'
										);
									}
								)
							: _('#at_biz_dir-categories').trigger('change')));
				var c = [];
				if (v.media_uploader) {
					var s,
						d = m(v.media_uploader);
					try {
						for (d.s(); !(s = d.n()).done; ) {
							var l = s.value;
							if (_('.' + l.element_id).length) {
								var u = new EzMediaUploader({
									containerClass: l.element_id,
								});
								(c.push({
									media_uploader: u,
									uploaders_data: l,
				...
				(_('body').on(
					...
					function (t) {
						...
						var e = _(t.target),
							i = {},
							r = _('.directorist-form-submit__btn');
						if (!S) {
							var n = [],
								a = [];
							if (c.length)
								for (var s = 0, d = c; s < d.length; s++) {
									...
									if (
										l.media_uploader &&
										_(l.media_uploader.container)
											.parents('form')
											.get(0) === e.get(0)
									) {
										if (!l.media_uploader.hasValidFiles()) {
											(r.removeClass('atbd_loading'),
												(i.listing_gallery = {
													msg: l.uploaders_data
														.error_msg,
												}),
												b(
													'.' +
							...
							if (n.length) {
								...
								function w() {
									...
									(t.append(
										...
										t.append('image', n[u]),
										t.append('image', n[u].file),
										t.append('field', n[u].field),
										_.ajax({
											method: 'POST',
											processData: !1,
											contentType: !1,
						...
						function g(t) {
							var e =
								...
								r = {},
								n = new FormData();
							(n.append('action', 'add_listing_action'),
								n.append(
									'directorist_nonce',
									directorist.directorist_nonce
								),
								f());
							var a,
								s = m(t.serializeArray());
							try {
								for (s.s(); !(a = s.n()).done; ) {
									var d = a.value;
									n.append(d.name, d.value);
								}
							} catch (t) {
								s.e(t);
							} finally {
							...
							e.length &&
								e.forEach(function (t) {
									n.append(''.concat(t.field, '[]'), t.file);
								});
							var w = t.find('#at_biz_dir-categories').val();
							...
							('string' == typeof w &&
								n.append('tax_input[at_biz_dir-category][]', w),
								n.has('admin_category_select[]') &&
									n.delete('admin_category_select[]'),
								n.has('directory_type') &&
									n.delete('directory_type'));
							var S = t.find("input[name='directory_type']"),
								A = void 0 !== S ? S.val() : '',
								L = o.directory_type ? o.directory_type : A;
							if (
								(n.append('directory_type', L),
								o.plan && n.append('plan_id', o.plan),
								o.order && n.append('order_id', o.order),
								i)
							)
								return (
									h(),
									console.log('Form has invalid data'),
									void console.log(i, r)
								);
							_.ajax({
								method: 'POST',
								processData: !1,
								...
								beforeSend: function () {
									(f(),
										C.show().html(
											'<span class="atbdp_success">'.concat(
												v.i18n_text.submission_wait_msg,
												'</span>'
											)
										));
								},
					...
					_(window).on('load', function () {
						_('.directorist-custom-field-btn-more').length &&
							_('.directorist-custom-field-btn-more').each(
								function (t, e) {
									var i = _(e).closest(
									...
									(_(r).slice(20, r.length).hide(),
										r.length <= 20 && _(e).hide());
								}
							);
					}),
					_('body').on(
						...
						function (t) {
							t.preventDefault();
							var e = _(this).closest(
									'.directorist-custom-field-checkbox, .directorist-custom-field-radio'
								),
								i = _(e).find(
									'.directorist-checkbox, .directorist-radio'
								);
							(_(i).slice(20, i.length).slideUp(),
								_(this).toggleClass('active'),
								_(this).hasClass('active')
									? (_(this).text(v.i18n_text.see_less_text),
										_(i).slice(20, i.length).slideDown())
									: (_(this).text(v.i18n_text.see_more_text),
										_(i).slice(20, i.length).slideUp()));
						}
					),
					_('#directorist-quick-login .directorist-toggle-modal').on(
						'click',
						function (t) {
							(t.preventDefault(),
								_('#directorist-quick-login').removeClass(
									'show'
								));
						}
					),
					_('#quick-login-from-submit-btn').on('click', function (t) {
						t.preventDefault();
						var e = _(_(this).data('form')),
							i = e.find('.directorist-modal-alerts-area');
						i = i.length ? i : e.find('.directorist-form-feedback');
						var r = e.find('input[name="email"]'),
					...
					_(window).scroll(
						((f = function () {
							var t = _(window).width(),
								e = _('.multistep-wizard__nav').width(),
								i = _('.multistep-wizard__nav').height(),
								r =
									_('.multistep-wizard').offset() &&
									_('.multistep-wizard').offset().top,
								n = _('.multistep-wizard').outerHeight();
							if (t > 991) {
								var o = _(window).scrollTop();
								o >= r
									? (_('.multistep-wizard__nav').addClass(
											'sticky'
										),
										_('.multistep-wizard__content').css(
									...
									: (_('.multistep-wizard__nav').removeClass(
											'sticky'
										),
										_('.multistep-wizard__content').css(
											'padding-inline-start',
			...
			_(window).on('elementor/frontend/init', function () {
				setTimeout(function () {
					_('body').hasClass('elementor-editor-active') && x();
				}, 3e3);
			}),
			_('body').on('click', function (t) {
				_('body').hasClass('elementor-editor-active') &&
					'A' !== t.target.nodeName &&
