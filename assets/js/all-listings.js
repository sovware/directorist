				...
				(function ($) {
					...
					function update_instant_search_url(form_data) {
						if (history.pushState) {
							...
							if (
								form_data.directory_type &&
								form_data.directory_type.length
							) {
								var query =
									query && query.length
										? query +
							...
							if (
								...
								Object.keys(form_data.custom_field).forEach(
									function (key) {
										query = query.length
											? query +
												'&'
					...
					$('body').on(
						...
						function (e) {
							...
							$(this)
								.find(
									'input[name^="search_by_rating["]:checked'
								)
								.each(function (index, el) {
							...
							var view_href = instant_search_element
								.find(
							...
							var view =
								view_as && view_as.length
									...
									: '';
							var type_href = instant_search_element
								.find(
							...
							var type =
								type_href && type_href.length
									...
									: '';
							var directory_type = getURLParameter(
								type_href,
								'directory_type'
							);
							var data_atts =
							...
							if (fields.zip && fields.zip.length) {
								fields.zip_cityLat = $(this)
									.find('.zip-cityLat')
									.val();
								fields.zip_cityLng = $(this)
									.find('.zip-cityLng')
									.val();
								fields.miles = $(this)
									.find('input[name="miles"]')
							...
							var form_data = _objectSpread(
								_objectSpread({}, data),
								fields
							);
							var allFieldsAreEmpty = Object.values(fields).every(
								function (item) {
									return !item;
								}
							);
							var tagFieldEmpty = data.in_tag.every(
								function (item) {
									return !item;
								}
							);
							var priceFieldEmpty = data.price.every(
								function (item) {
									return !item;
								}
							);
							var ratingFieldEmpty = data.search_by_rating.every(
								function (item) {
									return !item;
								}
							);
							var customFieldsAreEmpty = Object.values(
								data.custom_field
							).every(function (item) {
								return !item;
							});
								...
								$.ajax({
									...
									success: function success(html) {
										if (html.search_result) {
											...
											var website_name =
												directorist.site_name; // This is dynamically set from WordPress
											...
											if (String(html.location_name)) {
												if (
													String(html.category_name)
												) {
													new_meta_title +=
														' within ' +
														html.location_name; // If category exists, add with a comma
												} else {
													new_meta_title +=
														html.location_name; // If no category, just add location
												}
											}
											...
											if (fields.address) {
												if (
													...
													new_meta_title +=
														' near ' +
														fields.address; // If category or location exists, add "near"
												} else {
													new_meta_title +=
														fields.address; // Default to just the address
												}
											}
											...
											if (new_meta_title) {
												new_meta_title +=
													' | ' + website_name; // Append the website name only if the title has content
											} else {
					...
					$('body').on(
						...
						function (e) {
							if ($('.directorist-instant-search').length) {
								...
								if (
									$('input[name="open_now"]').is(':checked')
								) {
									fields.open_now = $(this)
										.find('input[name="open_now"]')
										.val();
								...
								var allFieldsAreEmpty = Object.values(
									fields
								).every(function (item) {
									return !item;
								});
								...
								var ratingFieldEmpty =
									data.search_by_rating.every(
										function (item) {
											return !item;
										}
									);
					...
					$('body').on(
						...
						function (e) {
							...
							var page_no = $(this)
								.closest('.page-numbers.current')
								.text();
							var data_atts =
								instant_search_element.attr('data-atts');

							// Select Active Form Based on Screen Size
							var advancedForm = instant_search_element.find(
								'.directorist-advanced-filter__form'
							);
							var searchForm = instant_search_element.find(
								'.directorist-search-form'
							);
							var sidebarListing = instant_search_element.find(
								'.listing-with-sidebar'
							);
							var activeForm = sidebarListing.length
								? instant_search_element
								: screen.width > 575
									? advancedForm
									: searchForm;
							...
							var in_cat = activeForm
								.find('.directorist-category-select')
								.val();
							var in_loc = activeForm
								.find('.directorist-location-select')
								.val();
							var price_range = activeForm
								.find("input[name='price_range']:checked")
								.val();
							var search_by_rating = activeForm
								.find('select[name=search_by_rating]')
								.val();
							var cityLat = activeForm.find('#cityLat').val();
							var cityLng = activeForm.find('#cityLng').val();
							var miles = activeForm
								.find('input[name="miles"]')
								.val();
							var address = activeForm
								.find('input[name="address"]')
								.val();
							var zip = activeForm
								.find('input[name="zip"]')
							...
							var fax = activeForm
								.find('input[name="fax"]')
								.val();
							var email = activeForm
								.find('input[name="email"]')
								.val();
							var website = activeForm
								.find('input[name="website"]')
								.val();
							var phone = activeForm
								.find('input[name="phone"]')
								.val();

							// Required fields Check
							var isQueryRequired = activeForm
								.find('input[name="q"]')
								.prop('required');
							var isCategoryRequired = activeForm
								.find('.directorist-category-select')
								.prop('required');
							var isLocationRequired = activeForm
								.find('.directorist-location-select')
								.prop('required');

							// Validate: If a field is required but empty, return false
							var requiredFieldsAreValid = true;
							if (isQueryRequired && !q)
								requiredFieldsAreValid = false;
							if (
								isCategoryRequired &&
								(!in_cat || in_cat.length === 0)
							)
								requiredFieldsAreValid = false;
							if (
								isLocationRequired &&
								(!in_loc || in_loc.length === 0)
							)
								requiredFieldsAreValid = false;
							$(
							...
							var form_data = {
								...
								custom_field:
									custom_field ||
									getURLParameter(full_url, 'custom_field'),
								data_atts: JSON.parse(data_atts),
							};
							...
							if (form_data.address && form_data.address.length) {
								form_data.cityLat = activeForm
									.find('#cityLat')
									.val();
								form_data.cityLng = activeForm
									.find('#cityLng')
									.val();
							...
							if (form_data.zip && form_data.zip.length) {
								form_data.zip_cityLat = activeForm
									.find('.zip-cityLat')
									.val();
								form_data.zip_cityLng = activeForm
									.find('.zip-cityLng')
									.val();
								form_data.miles = activeForm
									.find('input[name="miles"]')
							...
							$.ajax({
								...
								beforeSend: function beforeSend() {
									...
									instant_search_element
										.find(
											'.directorist-dropdown__links-js a'
										)
										.removeClass(
						...
						function (e) {
							...
							var form_data = {
								...
								in_loc:
									...
									getURLParameter(full_url, 'in_loc'),
								in_tag:
									tag || getURLParameter(full_url, 'in_tag'),
								price:
									price || getURLParameter(full_url, 'price'),
								price_range:
									price_range ||
									getURLParameter(full_url, 'price_range'),
								search_by_rating:
								...
								cityLng:
									...
									getURLParameter(full_url, 'cityLng'),
								miles:
									miles || getURLParameter(full_url, 'miles'),
								address:
									address ||
									getURLParameter(full_url, 'address'),
								...
								email:
									email || getURLParameter(full_url, 'email'),
								website:
									website ||
									getURLParameter(full_url, 'website'),
								phone:
									phone || getURLParameter(full_url, 'phone'),
								custom_field:
									custom_field ||
									getURLParameter(full_url, 'custom_field'),
								view: view,
					...
					$('body').on(
						...
						function (e) {
							...
							var $directory_nav = $container.find(
								'.directorist-type-nav__list'
							);
							var sort_href = $container
								.find(
									'.directorist-sortby-dropdown .directorist-dropdown__links__single.active'
								)
								.attr('data-link');
							...
							// Select Active Form Based on Screen Size
							var advancedForm = $container.find(
								'.directorist-advanced-filter__form'
							);
							var searchForm = $container.find(
								'.directorist-search-form'
							);
							var sidebarListing = $container.find(
								'.listing-with-sidebar'
							);
							var activeForm = sidebarListing.length
								? $container
							...
							var page_no = '';
							if (paginate_link) {
								var pageMatch = paginate_link.match(
									/(?:page\/|paged=)(\d+)/
								);
								if (pageMatch) {
					...
					function buildFormData(activeForm, instantSearchElement) {
						...
						var price = [];
						var customField = {};
						var dataAtts = JSON.parse(
							instantSearchElement.attr('data-atts')
						);
						activeForm
							.find('input[name^="in_tag["]:checked')
							.each(function (_, el) {
						...
						return {
							...
							directory_type:
								getDirectoryType(instantSearchElement),
							open_now: activeForm
								.find('input[name="open_now"]:checked')
								.val(),
						};
					}
					...
					function getSortValue(instantSearchElement) {
						var sortHref = instantSearchElement
							.find(
								'.directorist-sortby-dropdown .directorist-dropdown__links__single.active'
							)
							.data('link');
					...
					function getDirectoryType(instantSearchElement) {
						var typeHref = instantSearchElement
							.find(
								'.directorist-type-nav__list .directorist-type-nav__list__current a'
							)
							.attr('href');
						return typeHref
							? getURLParameter(typeHref, 'directory_type')
							: '';
					}
					...
					function filterListing(searchElm) {
						...
						searchElm
							...
							.each(function (index, el) {
								...
								if ('radio' === type) {
									...
									if (
										post_id.startsWith('custom-number') &&
										value === '0-0'
									) {
										return;
									}
						...
						var data_atts = $('.directorist-instant-search').attr(
							'data-atts'
						);
						var data = {
							action: 'directorist_instant_search',
						...
						if (fields.address && fields.address.length) {
							...
							fields.miles = searchElm
								.find('input[name="miles"]')
								.val();
						}
						if (fields.zip && fields.zip.length) {
							fields.zip_cityLat = searchElm
								.find('.zip-cityLat')
								.val();
							fields.zip_cityLng = searchElm
								.find('.zip-cityLng')
								.val();
							fields.miles = searchElm
								.find('input[name="miles"]')
						...
						$.ajax({
							...
							beforeSend: function beforeSend() {
								...
								$(_this)
									...
									.hide();
								if (
									$('.directorist-instant-search').offset() >
									0
								) {
									$(document).scrollTop(
										$(_this)
								...
								if (html.search_result) {
									...
									if (fields.address) {
										if (fields.in_cat || fields.in_loc) {
											new_meta_title +=
												' near ' + fields.address; // If category or location exists, add "near"
										} else {
