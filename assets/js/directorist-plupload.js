...
function plu_show_thumbs(imgId) {
  ...
  for (var i = 0; i < images.length; i++) {
    if (images[i] && images[i] != 'null') {
      ...
      var thumb = $('<div class="thumb ' + file_display_class + '" id="thumb' + imgId + i + '">' + image_title_html + file_display + image_caption_html + '<div class="atbdp-thumb-actions">' + '<span class="thumbremovelink" id="thumbremovelink' + imgId + i + '"><i class="directorist-icon-mask" style="--directorist-icon:url(' + directorist.assets_url + 'icons/font-awesome/svgs/solid/trash.svg);"></i></span>' + '</div>' + '</div>');
