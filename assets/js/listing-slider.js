...
***/
	(function ($) {
		...
		function allListingSlider() {
			...
			swiperCarouselListing.forEach(function (el, i) {
				...
				var swiperConfig = {
					...
					navigation: {
						nextEl: '.directorist-swiper__nav--next-listing-'.concat(
							i
						),
						prevEl: '.directorist-swiper__nav--prev-listing-'.concat(
							i
						),
					},
					pagination: {
						el: '.directorist-swiper__pagination--listing-'.concat(
							i
						),
						type: 'bullets',
						clickable: true,
				...
				var swiper = new Swiper(
					'.directorist-swiper-listing-'.concat(i),
					swiperConfig
				);
			});

			/* Swiper Slider Related Listing */
			var swiperCarouselRelated = document.querySelectorAll(
			...
			swiperCarouselRelated.forEach(function (el, i) {
				...
				navBtnPrev.forEach(function (el, i) {
					return el.classList.add(
						'directorist-swiper__nav--prev-related-'.concat(i)
					);
				});
				navBtnNext.forEach(function (el, i) {
					return el.classList.add(
						'directorist-swiper__nav--next-related-'.concat(i)
					);
				});
				swiperPagination.forEach(function (el, i) {
					return el.classList.add(
						'directorist-swiper__pagination--related-'.concat(i)
					);
				});
				el.classList.add(
					'directorist-swiper-related-listing-'.concat(i)
				);

				// Count slides directly from the DOM
				var relatedWrapper = el.querySelector('.swiper-wrapper');
				var totalSlides = relatedWrapper
					? relatedWrapper.children.length
					: 0;

				// Get Data Attribute
				var baseSlidesPerView = checkData(
					parseInt(el.dataset.swItems),
					4
				);
				var responsiveBreakPoints = checkData(
					el.dataset.swResponsive
						? JSON.parse(el.dataset.swResponsive)
						: undefined,
					{}
				...
				function initSwiper(loopValue) {
					...
					var config = {
						...
						navigation: {
							nextEl: '.directorist-swiper__nav--next-related-'.concat(
								i
							),
							prevEl: '.directorist-swiper__nav--prev-related-'.concat(
								i
							),
						},
						pagination: {
							el: '.directorist-swiper__pagination--related-'.concat(
								i
							),
							type: 'bullets',
							clickable: true,
				...
				function getCurrentSlidesPerView() {
					...
					if (responsiveBreakPoints) {
						var breakPoints = Object.keys(responsiveBreakPoints)
							.map(function (k) {
								return parseInt(k);
							})
							.sort(function (a, b) {
			...
			var singleListingSlider = document.querySelectorAll(
			...
			singleListingSlider.forEach(function (el, i) {
				...
				var dataBackgroundColor = el.getAttribute(
					'data-background-color'
				);
				var dataBackgroundSize = el.getAttribute(
					'data-background-size'
				);
				var dataBackgroundBlur = el.getAttribute(
					'data-blur-background'
				);
				var dataShowThumbnails = el.getAttribute(
					'data-show-thumbnails'
				);
				var dataThumbnailsBackground = el.getAttribute(
					'data-thumbnail-background-color'
				);

				// Find Sliders
				var swiperCarouselSingleListingThumb = el.querySelector(
					'.directorist-single-listing-slider-thumb'
				);
				var swiperCarouselSingleListing = el.querySelector(
					'.directorist-single-listing-slider'
				);

				// Single Listing Thumb Init
				var swiperSingleListingThumb = new Swiper(
				...
				var updateBlurredBackground =
					function updateBlurredBackground() {
						...
						if (!blurredBackground) {
							...
							blurredBackground.classList.add(
								'blurred-background'
							); // Add the class
						...
						if (activeSlide) {
							...
							blurredBackground.style.backgroundImage =
								'url('.concat(activeImageSrc, ')'); // Set as background image
		...
		window.addEventListener(
			...
			function () {
				if (
					$('.directorist-archive-items .directorist-swiper-listing')
				) {
					allListingSlider();
				}
			}
		...
		function sliderObserver() {
			var rangeSliders = document.querySelectorAll(
			...
			rangeSliders.forEach(function (rangeSlider) {
				if (rangeSlider) {
					...
					var observerCallback = function observerCallback(
						...
						var _iterator2 =
								_createForOfIteratorHelper(mutationList),
							_step2;
						try {
							for (
								_iterator2.s();
