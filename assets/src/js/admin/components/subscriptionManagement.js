window.addEventListener( 'load', () => {
			...
			success( response ) {
				...
				if ( response?.status?.log ) {
					for ( const feedback in response.status.log ) {
						...
						const alert_message =
							response.status.log[ feedback ].message;
				...
				if ( response?.status?.success ) {
					...
					var title =
						'<h3 class="h3 form-header-title">Activate your products</h3>';
					form_response_page
						.find( '.atbdp-form-feedback' )
						.append( title );
					...
					if ( total_theme ) {
						...
						form_response_page
							.find( '.atbdp-form-feedback' )
							.append( theme_section );

						const theme_title = `<h4 class="atbdp-theme-title">Themes <span class="atbdp-count">(${ themes.length })</span></h4>`;
					...
					if ( total_plugin ) {
						...
						const plugin_title = `<h4 class="atbdp-extension-title">Extensions <span class="atbdp-count">(${ plugins.length })</span></h4>`;
						...
						for ( const extension of plugins ) {
							...
							var checkbox = `<input type="checkbox" class="atbdp-checkbox atbdp-plugin-checkbox-item-${ extension.item_id }" value="${ extension.item_id }" id="${ extension.item_id }">`;
							var list_action = `<span class="atbdp-list-action">${ checkbox }</span> `;
							var label = `<label for="${ extension.item_id }">${ extension.title }</label>`;
							var li = `<li class="atbdp-check-list-item atbdp-plugin-checklist-item check-list-item-${ extension.item_id }">${ list_action }${ label }</li>`;

							form_response_page
								.find( '.atbdp-extensions-list' )
								.append( li );
							counter++;
						}
					}
					...
					$( '.atbdp-download-products-btn' ).on(
						'click',
						function ( e ) {
							...
							const new_plugin_count =
								total_plugin - skiped_plugins;
							...
							$(
								'.atbdp-check-list-item .atbdp-list-action .atbdp-checkbox'
							).css( 'display', 'none' );
							$(
								'.atbdp-check-list-item .atbdp-list-action'
							).prepend(
							...
							var download_files = function (
								...
								const is_checked = list_checkbox.is(
									':checked'
								);
								...
								jQuery.ajax( {
									...
									success( response ) {
										...
										if ( response.status.success ) {
											...
											const msg = `<span class="atbdp-list-feedback atbdp-text-danger">${ response.status.message }</span>`;
							...
							if ( new_theme_count || new_plugin_count ) {
								const form_header_title =
									'Activating your products';
								form_response_page
									.find(
										'.atbdp-form-feedback .form-header-title'
									)
							...
							const after_download_callback = function () {
								...
								const failed_download_themes_count =
									files_download_states.failed_theme_downloads
										.length;
								if (
									new_theme_count &&
									failed_download_themes_count ===
										new_theme_count
								) {
								...
								const failed_download_plugins_count =
									files_download_states
										.failed_plugin_downloads.length;
								if (
									new_plugin_count &&
									failed_download_plugins_count ===
										new_plugin_count
								) {
								...
								if (
									all_thmes_are_invalid &&
									all_plugins_are_invalid
								) {
	...
	$( '.et-close-auth-btn' ).on( 'click', function ( e ) {
		...
		const auth_section = $( '.et-auth-section' );
		const purchase_refresh_btn_wrapper = $(
	...
	$( '#atbdp-my-extensions-form' ).on( 'submit', function ( e ) {
		...
		const task = $( this ).find( 'select[name="bulk-actions"]' ).val();
		const plugins_items = [];
