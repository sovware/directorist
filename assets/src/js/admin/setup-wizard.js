...
jQuery( document ).ready( function ( $ ) {
		...
		var run_import = function () {
			...
			$.ajax( {
				...
				success( response ) {
					...
					$( '.directorist-importer-progress' ).val(
						response.percentage
					);
	...
	$( '.directorist-submit-importing' ).on( 'click', function ( e ) {
		...
		var import_dummy = function () {
			...
			if (
				$( 'input[name="directory_type_settings"]' ).is( ':checked' )
			) {
				data.directory_type_settings = true;
			}

			if (
				$( 'input[name="share_non_sensitive_data"]' ).is( ':checked' )
			) {
