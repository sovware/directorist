	...
	<div class="cptm-badges-manager">
		<div class="cptm-badges-card">
			<div class="cptm-badges-card__head">
				<h3>Badges</h3>
				<p>
					Create as many badges as you want. Each badge has its own
					conditions and style. A listing can match multiple badges at
					once.
				</p>
			</div>
			...
			<div class="cptm-badges-toolbar">
				<div class="cptm-badges-toolbar__info">
					Tip: keep each badge's conditions simple. One badge answers
					one question.
				</div>
			...
			<div class="cptm-badges-list">
				...
				>
					...
					>
						<span class="cptm-badge-item__left">
							<span
								...
								:class="{
									'cptm-badge-chip--icon':
										badgeType(badge) === 'icon',
									'cptm-badge-chip--has-icon':
					...
					<div
						v-show="isOpen(badge.key)"
						class="cptm-badge-item__body"
					>
							...
							>
								...
								>
									<input
										...
										"
										@change="
											updateBadgeType(badge, option.value)
										"
									/>
									<span
										class="cptm-badges-type-option__title"
										>{{ option.label }}</span
									>
									<span
										class="cptm-badges-type-option__description"
						...
						<div class="cptm-badge-edit-grid">
							...
							<div
								class="cptm-badge-field cptm-badge-field--icon-picker"
							>
								<span class="cptm-badge-field__label"
									>Icon</span
								>
							...
							>
								...
								<span class="cptm-badge-tooltip-colors">
									<span
										class="cptm-badge-tooltip-colors__item"
									>
										<span class="cptm-badge-color-input">
											<input
												type="color"
								...
								<span class="cptm-badge-field__help">
									Second swatch controls the tooltip text
									color.
								</span>
							</div>
						</div>
						...
						<div class="cptm-badge-match">
							<div class="cptm-badge-match__head">
								...
								<span class="cptm-badge-preview-hint"
									>Live preview</span
								>
							</div>
...
export default {
	...
	computed: {
		...
		badgeRuleConditionSources() {
			...
			return (
				ruleField.condition_sources || ruleField.conditionSources || {}
			);
		},
		...
		pricingConditionOptions() {
			...
			return [
				...
				},
				{
					...
					defaultValue: planOptions.length
						? planOptions[0].value
						: '',
	...
	methods: {
		...
		normalizeSourceOption(option, fallbackValueType = 'text') {
			...
			const hasDefaultValueAlias =
				typeof option.default_value !== 'undefined';

			return {
				value: this.stringifyValue(option.value),
				label: this.stringifyValue(option.label, option.value),
				valueType,
				defaultOperator: this.stringifyValue(
					option.defaultOperator || option.default_operator,
		...
		badgeRuleSourceFieldsChanged() {
			return this.badgeRuleSourceKeys().some(
				(fieldKey) =>
					...
					this.cachedFieldValue(fieldKey),
			);
		},
		...
		initialRulesDraft() {
			const savedRules = this.normalizeBadgeRules(
				this.parseBadgeRules(
					this.rawFieldValue('directorist_badge_rules'),
				),
				false,
			);
		...
		normalizedStyle(
			style = {},
			fallbackColor = '#3e62f5',
			fallbackStyle = {},
		) {
			const bg =
				this.normalizeHex(style.bg || fallbackColor) || fallbackColor;
			const text =
				this.normalizeHex(style.text || fallbackStyle.text) ||
				'#ffffff';
			const border =
		...
		normalizedHover(hover = {}, fallbackHover = {}) {
			...
			const fallback =
				fallbackHover && typeof fallbackHover === 'object'
					? fallbackHover
					: {};
		...
		normalizedCondition(condition) {
			...
			const source = this.sourceIsAllowed(
		...
		conditionKeyOptions(source, key = '') {
			const options = this.conditionOptionsForSource(source).map(
				(option) => ({
					value: option.value,
					label: option.label,
				}),
		...
		operatorAlias(operator) {
			...
			return this.stringifyValue(operator, '=');
		},
		...
		badgeColorControlLabel(badge) {
			return this.badgeType(badge) === 'icon'
				? 'Icon color'
				: 'Text color';
		},
		...
		updateBadgeIcon(badge, value) {
			...
			this.$set(
				rule,
				'icon',
				shouldRestoreIcon ? badge.defaultIcon : icon,
			);
		...
		chipLabel(badge) {
			return (
				this.badgeVisibleLabel(badge) || this.badgeInternalName(badge)
			);
		},
		...
		conditionSummary(badge) {
			...
			const glue =
				this.badgeMatchMode(badge) === 'any' ? ' OR ' : ' AND ';
