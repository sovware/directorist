...
export default {
	...
	methods: {
		syncLayoutWithWidgets() {
			...
			if (!(this.local_layout && typeof this.local_layout === 'object')) {
				return;
			}
			...
			for (let section_key in this.local_layout) {
				...
				for (let sub_section_key in section) {
					...
					if (!(sub_section && typeof sub_section === 'object')) {
						continue;
					}
					if (
						!(
					...
					for (let widget_key of sub_section.selectedWidgets) {
						if (!deprecated_widgests_keys.includes(widget_key)) {
							continue;
						}

						let _index =
							sub_section.selectedWidgets.indexOf(widget_key);
