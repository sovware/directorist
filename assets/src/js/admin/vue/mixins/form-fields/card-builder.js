...
export default {
	...
	methods: {
		syncLayoutWithWidgets() {
			let available_widgets_keys = Object.keys(
			...
			if (
				! ( this.local_layout && typeof this.local_layout === 'object' )
			) {
				return;
			}
			...
			for ( let section_key in this.local_layout ) {
				...
				for ( let sub_section_key in section ) {
					...
					if (
						! ( sub_section && typeof sub_section === 'object' )
					) {
						continue;
					}
					if (
						! (
							sub_section.selectedWidgets &&
							Array.isArray( sub_section.selectedWidgets )
						)
					) {
						continue;
					}
					...
					for ( let widget_key of sub_section.selectedWidgets ) {
						if (
							! deprecated_widgests_keys.includes( widget_key )
						) {
							continue;
						}

						let _index = sub_section.selectedWidgets.indexOf(
							widget_key
						);
						this.local_layout[ section_key ][
							sub_section_key
						].selectedWidgets.splice( _index, 1 );
