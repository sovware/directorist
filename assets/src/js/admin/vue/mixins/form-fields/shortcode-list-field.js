	...
	methods: {
				...
				for ( let shortcode_item of this.shortcodes ) {
					...
					if ( typeof shortcode_item === 'object' ) {
						...
						if ( shortcode_item.mapAtts ) {
							_shortcode = this.applyAttsMapping(
								shortcode_item
							);
						}
						...
						if ( Array.isArray( _shortcode ) ) {
							this.shortcodes_list = this.shortcodes_list.concat(
								_shortcode
							);
						}
					}
		...
		applyAttsMapping( shortcode_args ) {
			...
			for ( let map of shortcode_args.mapAtts ) {
				...
				if ( map.mapAll ) {
					mapped_shortcode = this.applyMapAll(
						map,
						mapped_shortcode
					);
				}
		...
		applyMap( args, value ) {
			var shortcode = value;
			const source = this.getTergetFields( {
				root: this.root,
				path: args.map,
			} );
