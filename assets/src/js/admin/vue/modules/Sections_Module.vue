			...
			<div class="cptm-form-fields" v-if="sectionFields(section)">
				<template v-for="(field, field_key) in sectionFields(section)">
					<settings-default-location-address
						...
						@update-field="
							updateFieldValue($event.fieldKey, $event.value)
						"
					/>
					...
					<settings-checkout-currency-match
						...
						@set-custom-mode="
							setCheckoutCurrencyCustomMode(section_key, $event)
						"
						@update-field="
							updateFieldValue($event.fieldKey, $event.value)
						"
					...
					>
						<button
							...
							:aria-expanded="
								isNestedAdvancedOpen(section_key)
									? 'true'
									: 'false'
							"
					...
					<settings-badges-manager
						v-if="shouldRenderBadgesManager(section, field)"
						:key="'badges-manager-' + section_key"
						:fields="fields"
					...
					>
						...
						<component
							...
							@validate="
								updateFieldValidationState(field, $event)
							"
							@is-visible="
								updateFieldData(field, 'isVisible', $event)
							"
						...
						>
							<div class="cptm-preview-notice-content">
								...
								<p class="cptm-preview-notice-text">
									...
									<strong>Grid</strong>,
									<strong>List</strong> or
									<strong>Map</strong> views for the All
									Listings Page?
								</p>
							</div>
						...
						>
							<div class="atbdp-row">
								<div class="atbdp-col atbdp-col-4">
									<label
										class="cptm-field-group-container__label"
									>
										<span>{{
								...
								<div class="atbdp-col atbdp-col-8">
									<div class="cptm-container-group-fields">
										<component
											...
											) in sectionGroupedContainerFields(
												section,
											)"
											:is="
												getFormFieldName(
													fields[groupedField].type,
												)
											"
...
import SettingsBadgesManager from '../apps/settings-manager/components/Settings_Badges_Manager.vue';
...
export default {
	...
	methods: {
		...
		sectionShouldRender(section, sectionKey = '') {
			...
			const showIf =
				section.showIf || section.show_if || section['show-if'];
		...
		nestedAdvancedToggleLabel(section) {
			return section && section.advancedLabel
				? section.advancedLabel
				: 'Advanced';
		},

		nestedAdvancedToggleClass(sectionKey) {
			return {
				'cptm-card-advanced--open':
					this.isNestedAdvancedOpen(sectionKey),
				'cptm-card-advanced--collapsed':
					!this.isNestedAdvancedOpen(sectionKey),
			};
		},
		...
		isNestedAdvancedOpen(sectionKey) {
			return !!this.nestedAdvancedOpen[
				this.nestedAdvancedKey(sectionKey)
			];
		},
		...
		toggleNestedAdvanced(sectionKey) {
			...
			this.$set(
				this.nestedAdvancedOpen,
				key,
				!this.nestedAdvancedOpen[key],
			);
		},
		...
		shouldRenderBadgesManager(section, field) {
			if (!section || !section.badgesManager) {
				return false;
			}

			return section.badgesManager.beforeField === field;
		},
		...
		checkoutCurrencyMatchesDisplay() {
			...
			const checkoutCurrency = String(
				this.fields.payment_currency?.value || '',
			)
				.trim()
				.toUpperCase();
			const displayPosition = String(
				this.fields.g_currency_position?.value || '',
			);
		...
		isCheckoutCurrencyField(field) {
			return ['payment_currency', 'payment_currency_position'].includes(
				field,
			);
		},
		...
		shouldRenderRestrictedCountriesSelect(field) {
			const isMapSettings =
				this.menuKey === 'listing_settings__map' ||
				this.tabKey === 'map';

			return isMapSettings && field === 'restricted_countries';
		},
		...
		shouldRenderEmailNotificationToggle(field) {
			...
			return (
				isNotificationChannels && field === 'disable_email_notification'
			);
		},
		...
		shouldRenderWebPushSetupCard(field) {
			...
			return (
				...
				field === 'web_push_admin_subscription'
			);
		},
		...
		containerGroupLabel(section) {
			const firstContainerField =
				this.sectionGroupedContainerFields(section)[0];
		...
		getSectionHighlightState(sectionKey) {
			return (
				...
				this.sectionHighlightKey(sectionKey)
			);
		},
		...
		sectionTitleAreaClass(section) {
			return {
				...
				'cptm-text-center':
					'center' === section.title_align ? true : false,
			};
		},
		...
		fieldWrapperClass(field_key, field, section = null) {
			...
			return {
				...
				'cptm-field-wraper--nested-advanced':
					this.isNestedAdvancedField(section, field_key),
			};
		},
		...
		fieldWrapperID(field) {
			...
			if (field && field.editor !== undefined) {
				type_id =
					field.editor === 'wp_editor' ? 'cptm-field_wp_editor' : '';
			}
			return type_id;
		},
