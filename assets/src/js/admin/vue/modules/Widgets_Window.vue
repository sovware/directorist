  ...
  <div class="cptm-option-card" :class="mainWrapperClass">
    ...
    <div class="cptm-option-card-body">
      ...
      <ul
        ...
        v-if="Object.keys(unSelectedWidgetsList).length"
      >
        <li
  ...
  computed: {
    widgetsList() {
      ...
      if (this.rejectedWidgets && this.rejectedWidgets.length) {
        availableWidgets = Object.keys(availableWidgets)
          .filter(
            (key) =>
              !this.rejectedWidgets.includes(availableWidgets[key].widget_name)
          )
          .reduce((obj, key) => {
      ...
      let widgets_list = Object.keys(availableWidgets)
        .filter((key) =>
          accepted_widgets.includes(availableWidgets[key].widget_name)
        )
        .reduce((obj, key) => {
          obj[key] = availableWidgets[key];
    ...
    unSelectedWidgetsList() {
      ...
      let widgets_list = Object.keys(self.widgetsList)
        .filter(
          (key) =>
            !self.localSelectedWidgets.includes(key) &&
            typeof self.activeWidgets[key] === "undefined"
        )
        .reduce((obj, key) => {
    ...
    maxWidgetLimitIsReached() {
      return (
        this.maxWidget && this.localSelectedWidgets.length >= this.maxWidget
      );
    },
  ...
  methods: {
    ...
    widgetListClass(widget_key) {
      return {
        ...
        disabled:
          this.maxWidgetLimitIsReached ||
          typeof this.activeWidgets[widget_key] !== "undefined",
