...
$( function () {
	...
	$( 'body' ).on( 'click', '#addNewSocial', function ( e ) {
		...
		const $container = $( '#social_info_sortable_container' );
		const template = $( '#tmpl-directorist-listing-form-social-item' );
		if ( template.length ) {
			const socialFieldsCount = $container.find(
	...
	function addFirstSocialItem() {
		if (
			! $( '#social_info_sortable_container' ).find(
				'.directorist-form-social-fields'
			).length
		) {
	...
	function renderCategoryCustomFields() {
		...
		if ( $watchableSections.hide.size ) {
			for ( const hidable of $watchableSections.hide ) {
				...
				if (
					$hidable.find( '.directorist-form-group:visible' ).length
				) {
