  ...
  $("body").on("submit", "#directorist-add-listing-form", function (e) {
    ...
    if (mediaUploaders.length) {
      for (var uploader of mediaUploaders) {
        if (
          ...
          $(uploader.media_uploader.container).parents("form").get(0) !==
            $form.get(0)
        ) {
          continue;
        }
        ...
        $.ajax({
          ...
          beforeSend() {
            ...
            if (totalImages === 1) {
              ...
              const completedPercent = Math.ceil(
                ((counter === 0 ? 1 : counter) * 100) / totalImages,
              );
              $notification
                .show()
                .html(
          ...
          success(response) {
            if (!response.success) {
              ...
              $notification
                .show()
                .html(`<span class="atbdp_error">${response.data}</span>`);
    ...
    function submitForm($form, uploadedImages = []) {
      ...
      var form_directory_type_value =
        form_directory_type !== undefined ? form_directory_type.val() : "";
      ...
      $.ajax({
        ...
        success(response) {
          var redirect_url =
            response && response.redirect_url ? response.redirect_url : "";
          redirect_url =
            redirect_url && typeof redirect_url === "string"
              ? response.redirect_url.replace(/:\/\//g, "%3A%2F%2F")
              : "";
      ...
      debounce(function () {
        ...
        if (windowWidth > 991) {
          ...
          if (scrollPos >= multiStepWizardOffset) {
            ...
            if (
              scrollPos >=
              multiStepWizardOffset + multiStepWizardHeight - sidebarHeight
            ) {
