...
export function setupFileUploadHandlers($, triggerFn) {
		...
		mutations.forEach(function (mutation) {
			if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
				...
				if ($target.hasClass('ezmu__preview-section') && $target.hasClass('ezmu--show')) {
					const $imageWrapper = $target.closest('.directorist-form-image-upload-field');
			...
			if (mutation.addedNodes.length > 0) {
				mutation.addedNodes.forEach(function (node) {
					...
					if ($node.hasClass('thumb') || $node.closest('.plupload-thumbs').length || $node.find('.thumb').length) {
						const $fileWrapper = $node.closest('.directorist-form-group, .directorist-custom-field-file-upload');
					...
					if ($node.hasClass('ezmu__preview-section') || $node.hasClass('ezmu--show') || $node.closest('.ezmu__preview-section.ezmu--show').length) {
						const $imageWrapper = $node.closest('.directorist-form-image-upload-field');
						if ($imageWrapper.length) {
							setTimeout(function () {
								triggerFn('listing_img', 'listing_img', $imageWrapper.find('.ez-media-uploader').first());
							}, 200);
				...
				mutation.removedNodes.forEach(function (node) {
					...
					if ($node.hasClass('thumb') || $node.closest('.plupload-thumbs').length || $node.find('.thumb').length) {
						...
						if ($thumbsContainer.hasClass('plupload-thumbs') || $thumbsContainer.find('.plupload-thumbs').length) {
							const $fileWrapper = $thumbsContainer.closest('.directorist-form-group, .directorist-custom-field-file-upload');
							if ($fileWrapper.length) {
								const fieldKey = resolveFileFieldKey($fileWrapper);
								if (fieldKey) {
									setTimeout(function () {
					...
					if ($node.hasClass('ezmu__file-item') || $node.hasClass('ezmu__new-file') || $node.closest('.ez-media-uploader').length || $node.hasClass('ezmu__old-files-meta') || $node.find('.ezmu__file-item, .ezmu__new-file').length) {
						...
						if ($uploaderContainer.hasClass('ez-media-uploader') || $uploaderContainer.closest('.ez-media-uploader').length) {
							const $imageWrapper = $uploaderContainer.closest('.directorist-form-image-upload-field');
							if ($imageWrapper.length) {
								setTimeout(function () {
									triggerFn('listing_img', 'listing_img', $imageWrapper.find('.ez-media-uploader').first());
								}, 300);
	...
	document.addEventListener(
		...
		function (e) {
			...
			const $removeButton = $target.closest('.thumbremovelink').length ? $target.closest('.thumbremovelink') : $target.hasClass('thumbremovelink') ? $target : null;
			if (!$removeButton || !$removeButton.length) return;

			const $thumb = $removeButton.closest('.thumb');
			if (!$thumb.length) return;

			const $fileWrapper = $thumb.closest('.directorist-form-group, .directorist-custom-field-file-upload');
			if (!$fileWrapper.length) return;

			const fieldKey = resolveFileFieldKey($fileWrapper);
			if (fieldKey) {
				setTimeout(function () {
					const $hiddenInput = $fileWrapper.find(`input[type="hidden"][name="${fieldKey}"], input[type="hidden"][name="${fieldKey}[]"]`).first();
...
function resolveFileFieldKey($fileWrapper) {
	let fieldKey = $fileWrapper.attr('data-field-key') || $fileWrapper.find('[data-field-key]').first().attr('data-field-key');
	if (!fieldKey) {
