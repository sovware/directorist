      ...
      if (directorist.i18n_text.select_listing_map === "google") {
                ...
                geocoder.geocode(
                  ...
                  function (results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                      if (results[0]) {
                        let add = formatAddress(results[0]);
                        let value = add.split(",");
        ...
        function displayLocation(position, event) {
          ...
          $.ajax({
            ...
            data: {},
            success(data) {
              $(".directorist-location-js, .atbdp-search-address").val(
                data.display_name,
              );
              $(".directorist-location-js, .atbdp-search-address").attr(
                "data-value",
                data.display_name,
              );
              $("#cityLat").val(lat);
