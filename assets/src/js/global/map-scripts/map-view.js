		...
		(function ($) {
			...
			function atbdp_rander_map($el) {
				...
				if ($markers.length) {
					cord.lat = defCordEnabled
						? Number(mapData.default_latitude)
						: Number($markers[0].getAttribute('data-latitude'));
					cord.lng = defCordEnabled
						? Number(mapData.default_longitude)
						: Number($markers[0].getAttribute('data-longitude'));
				}
			...
			function atbdp_add_marker($marker, map, infowindow) {
				...
				if (map.markers.length) {
					for (let i = 0; i < map.markers.length; i++) {
						...
						if (latlng.equals(pos)) {
							...
							const latitude =
								latlng.lat() + (Math.random() - 0.5) / 1500; // * (Math.random() * (max - min) + min);
							const longitude =
								latlng.lng() + (Math.random() - 0.5) / 1500; // * (Math.random() * (max - min) + min);
				...
				const marker = new Marker({
					...
					map_icon_label:
						icon !== undefined &&
						`<div class="atbd_map_shape">${icon}</div>`,
				});
			...
			function setup_info_window() {
				...
				document.querySelectorAll('div').forEach((el1, index) => {
					if (
						el1.style.backgroundImage.split('/').pop() ===
						'm1.png")'
					) {
