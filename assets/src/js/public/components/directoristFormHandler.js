		...
		this.ATBDP_Form_Handler = function ( args ) {
			...
			this.handleForm = function ( form, form_key ) {
				...
				$( form ).on( 'submit', function ( e ) {
					...
					const form = e.target;
					const is_processesing = $( form ).attr(
						'data-is-processesing'
					);
					...
					self.allFormData[ form_key ].formData.append(
						'action',
						form_action
					);
			...
			this.sendRequest = function ( form_key ) {
				...
				const handler =
					'handler' in this.allFormData[ form_key ]
						? self.allFormData[ form_key ].handler
						: false;
				const has_success_handler = !! (
					handler && 'success' in handler
				);
			...
			this.onSuccess = function ( response, form_key ) {
				...
				const response_is_json =
					response && typeof response === 'object';
				const response_is_string =
				...
				success_msg =
					'message' in response ? response.message : success_msg;
				error_msg =
					'message' in response ? response.message : error_msg;
			...
			this.onError = function ( response, form_key ) {
				const response_is_string =
					response && typeof response === 'string';
				const msg = response_is_string
					? response
					: this.option.message.error;
