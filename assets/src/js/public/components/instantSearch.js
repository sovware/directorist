...
(function ($) {
  ...
  function buildFormData(searchElm) {
    ...
    searchElm.find(".directorist-custom-range-slider__wrap").each(function () {
      const $wrap = $(this);
      const rangeField = $wrap.find(".directorist-custom-range-slider__range");
    ...
    const candidates = [
      {
        form: forms.advanced,
        hasDirectoryType:
          forms.advanced.find('input[name="directory_type"]').length > 0,
      },
      {
        form: forms.search,
        hasDirectoryType:
          forms.search.find('input[name="directory_type"]').length > 0,
      },
