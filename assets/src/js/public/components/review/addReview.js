	...
	window.addEventListener('load', () => {
		...
		function prepear_form_data(form, field_map, data) {
			...
			for (var key in field_map) {
				...
				if (field.length) {
					var data_key =
						'name' === field_type ? field_key : field.attr('name');
		...
		$('#directorist-review-form').on('submit', function (e) {
			...
			if (submit_count > 1) {
				// show error message
				swal({
					title: directorist.warning,
					text: directorist.not_add_more_than_one,
					type: 'warning',
					timer: 2000,
			...
			jQuery.post(directorist.ajaxurl, _data, function (response) {
				var output = '';
				var deleteBtn = '';
				var d;
				...
				var approve_immediately = $form
					.find('#approve_immediately')
					.val();
				...
				if (approve_immediately === 'no') {
					if (content === '') {
						...
						if (submit_count === 1) {
							$('#directorist-client-review-list').prepend(
								output
							); // add the review if it's the first review of the user
							$('.atbdp_static').remove();
						}
						submit_count++;
						if (review_duplicate === 'yes') {
							swal({
								title: directorist.warning,
								text: directorist.duplicate_review_error,
								type: 'warning',
								timer: 3000,
								showConfirmButton: false,
							});
							...
							swal({
								title: directorist.success,
								text: directorist.review_approval_text,
								type: 'success',
								timer: 4000,
								showConfirmButton: false,
							});
					...
					output +=
						'<div class="directorist-single-review" id="directorist-single-review-' +
						response.data.id +
						'">' +
						...
						'<div class="directorist-single-review-avatar">' +
						ava_img +
						'</div> ' +
						...
						name +
						'</p>' +
						'<span class="directorist-single-review-time">' +
						response.data.date +
						'</span> ' +
						'</div> ' +
						'</div> ' +
					...
					deleteBtn +=
						'<button class="directory_btn btn btn-danger" type="button" id="atbdp_review_remove" data-review_id="' +
						response.data.id +
						'">Remove</button>';
					...
					swal({
						title: directorist.review_success,
						type: 'success',
						timer: 800,
						showConfirmButton: false,
					});
