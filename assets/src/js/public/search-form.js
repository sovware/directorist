          ...
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              const dropdowns = node.querySelectorAll
                ...
                : node.matches && node.matches(this.options.dropdownClass)
                  ? [node]
                  : [];
...
(function ($) {
  window.addEventListener("load", () => {
    ...
    $(
      ...
      let checkedCheckbox = $(this).find('input[type="checkbox"]:checked');
    ...
    function directorist_custom_range_slider() {
      ...
      sliders.forEach(function (sliderItem) {
        ...
        function updateSliderFromInputs() {
          let minValue =
            Math.round(parseInt(minInput.value || 0, 10) / sliderStep) *
            sliderStep;
          let maxValue =
            Math.round(parseInt(maxInput.value || 0, 10) / sliderStep) *
            sliderStep;
