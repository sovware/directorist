...
( function ( $ ) {
	window.addEventListener( 'load', () => {
		...
		function selectedItemCount( item ) {
			let dropdownParent = $( item ).closest(
		...
		function initSearchFields() {
			...
			searchFields.forEach( ( searchField ) => {
				...
				if ( searchField.classList.contains( 'directorist-select' ) ) {
					inputFieldValue = searchField.querySelector( 'select' )
						.dataset.selectedId;
				}
		...
		function adsFormReset( searchForm ) {
			...
			let irisPicker = searchForm.querySelector(
		...
		$( 'body' ).on( 'click', '.directorist-modal-btn', function ( e ) {
			...
			if (
				this.classList.contains( 'directorist-modal-btn--advanced' )
			) {
				let searchModalElement = parentElement.querySelector(
		...
		$( 'body' ).on(
			...
			function ( e ) {
				...
				if ( windowScreen <= 575 ) {
					if (
						parentField.classList.contains( 'input-is-focused' )
					) {
		...
		$( 'body' ).on(
			...
			function ( event ) {
				event.preventDefault();
				let zipcode_search = $( this ).closest(
		...
		function directorist_custom_range_slider() {
			...
			sliders.forEach( function ( sliderItem ) {
				...
				let sliderMinValue = parseInt(
					slider.getAttribute( 'min-value' )
				);
				let sliderMaxValue = parseInt(
		...
		function rangeSliderObserver() {
			...
			targetNodes.forEach( ( targetNode ) => {
				if ( targetNode ) {
					...
					let sliderObserver = new MutationObserver(
						observerCallback
					);
