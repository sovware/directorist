  ...
  <div class="cptm-form-builder cptm-row">
    <div class="cptm-col-6">
      <div class="cptm-form-builder-active-fields">
        ...
        <button type="button" class="cptm-btn" v-if="showGroupDragToggleButton" :class="( forceExpandStateTo ) ? 'cptm-btn-primary' : ''" @click="toggleEnableWidgetGroupDragging">
            {{ ( forceExpandStateTo ) ? 'Disable Section Dragging' : 'Enable Section Dragging' }}
        </button>
        
        <div class="cptm-form-builder-active-fields-container">
            <draggable-list-item-wrapper list-id="widget-group" :is-dragging-self="( currentDraggingGroup && widget_group_key === currentDraggingGroup.widget_group_key )" :droppable="currentDraggingGroup" v-for="( widget_group, widget_group_key ) in active_widget_groups" :key="widget_group_key" @drop="handleGroupDrop( widget_group_key, $event )">
            ...
            <div class="cptm-form-builder-active-fields-footer" v-if="showAddNewGroupButton">
              <button type="button" class="cptm-btn cptm-btn-secondery" @click="addNewGroup" v-html="addNewGroupButtonLabel"></button>
            </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    groupSettingsProp() {
        if ( ! this.generalSettings ) { return this.groupSettings;  }
        if ( typeof this.generalSettings.minGroup === 'undefined' ) { return this.groupSettings;  }
  ...
  methods: {
    ...
    setupActiveWidgetGroups() {
        ...
        if ( Array.isArray( this.value.groups ) ) { this.active_widget_groups = this.value.groups; }
    ...
    updateGroupField( widget_group_key, payload ) {
      Vue.set( this.active_widget_groups[ widget_group_key ], payload.key, payload.value );
    ...
    updateWidgetField( payload ) {
      Vue.set( this.active_widget_fields[ payload.widget_key ], payload.payload.key, payload.payload.value );
    ...
    handleAppendWidget( widget_group_key ) {
      if ( ! this.currentDraggingWidget ) { return; }
    ...
    handleWidgetInsertFromAvailableWidgets( from, to ) {
      ...
      Vue.set( this.active_widget_fields, inserting_field_key, field_data_options );
      ...
      this.active_widget_groups[ to.widget_group_key ].fields.splice( dest_index, 0, inserting_field_key );
    ...
    trashWidget( widget_group_key, payload ) {
      let index = this.active_widget_groups[ widget_group_key ].fields.indexOf( payload.widget_key );
    ...
    handleGroupDrop( widget_group_key, payload ) {
        ...
        let dropped_in = { widget_group_key, drop_direction: payload.drop_direction };
