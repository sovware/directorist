window.IconPicker = function (args) {
	return {
		...
		init: function () {
			...
			const count = body.getAttribute(
				'data-directorist-icon-picker-count'
			);
			...
			this.id = id;
			this.container =
				typeof args.container !== 'undefined'
					? args.container
					: this.container;
			this.labels =
				typeof args.labels !== 'undefined'
					? {
							...this.labels,
							...args.labels,
						}
					: this.labels;
			this.onSelect =
				typeof args.onSelect !== 'undefined'
					? args.onSelect
					: this.onSelect;
			this.icons =
				typeof args.icons !== 'undefined' ? args.icons : this.icons;
			this.value =
		...
		renderIcon() {
			const iconsGroup = this.container
				.closest('body')
				.querySelector('#iconsWrapperElm .icons-group');
			...
			for (const iconGroupKey of Object.keys(this.icons)) {
				...
				for (const icon of this.icons[iconGroupKey].icons) {
					const fullIcon = this.getFullIcon(
						icon.key,
						iconGroupKey,
						icon.types[0]
					);
			...
			this.container
				.closest('body')
				.querySelector('#iconsWrapperElm').innerHTML = markup;
		},
		renderMarkup() {
			...
			const safeDisplayValue = String(displayValue).replace(
				/"/g,
				'&quot;'
			);
			markup += `
			...
			this.container
				.closest('body')
				.insertAdjacentHTML('beforeend', iconPickerWrap);
		},
		...
		getIconGroupOptions: function () {
			...
			for (const iconGroup in this.icons) {
				options +=
					`<option value="${iconGroup}">${this.getIconGroupLabel(iconGroup)}</option>` +
					'\n';
			}
		...
		getIconGroupLabel(iconGroup) {
			const labels =
				this.labels.iconGroupLabels &&
				typeof this.labels.iconGroupLabels === 'object'
					? this.labels.iconGroupLabels
					: {};
			return typeof labels[iconGroup] !== 'undefined'
				? labels[iconGroup]
				: this.icons[iconGroup].label;
		},
		...
		attachEvents() {
			...
			iconButtons.forEach((elm) => {
				elm.addEventListener('click', function (event) {
					...
					const iconGroupKey =
						event.currentTarget.getAttribute('data-group-key');
					const iconKey =
						event.currentTarget.getAttribute('data-icon-key');
					const iconType = event.currentTarget
					...
					if (body) {
						body.querySelector(
							...
							'class',
							`icon-picker__preview-icon ${icon}`
						);
						body.querySelector(
			...
			function openModal(event, self) {
				...
				const id = event.target
					.closest('.icon-picker-selector')
					.getAttribute('data-icon-picker-id');
				const selectedIconClassList = self.value
					? self.value.split(' ')
					: [];
				...
				const iconFilterSelect = iconPicker.querySelector(
					'.icon-picker__filter_select'
				);
				const iconType = selectedIconClassList.length
					? selectedIconClassList[0]
					: '';
				...
				const selectedIconClasses = selectedIconClassList.length
					? '.font-icon-btn.' + selectedIconClassList.join('.')
					: '';
				const selectedIcon =
					selectedIconClasses !== ''
						? iconPicker.querySelector(selectedIconClasses)
						: '';

				document
					.querySelectorAll('.font-icon-btn')
					.forEach((item) =>
						item.classList.remove('cptm-btn-primary')
					);
				...
				// Update Preview Icon
				const previewContainer = iconPicker.querySelector(
					'.icon-picker__preview'
				);
				previewContainer
					.querySelector('.icon-picker__preview-icon')
					.setAttribute(
						'class',
						'icon-picker__preview-icon ' + self.value
					);
				previewContainer.querySelector(
					'.icon-picker__icon-name'
				).innerHTML = selectedIconClassList.length
					? selectedIconClassList[1]
					: '';
				...
				for (const resetBtn of resetBtns) {
					...
					resetBtn.addEventListener('click', (e) => {
						...
						if (inputSibling) {
							...
							inputSibling.style.cssText =
								'padding-left: 20px; pointer-events: all; cursor: pointer;';
						}

						// Update classname of the directorist-selected-icon
						parent
							.querySelector('.directorist-selected-icon')
							.setAttribute('class', `directorist-selected-icon`);
			...
			document
				.querySelector('.icon-picker__done-btn')
				.addEventListener('click', (e) => {
					...
					const selector = document.querySelector(
						`.icon-picker-selector.icon-picker-id-${id}`
					);
					...
					selector.querySelector('input').style.cssText =
						'padding-left: 38px; pointer-events: none; cursor: auto;';
					selector
						.querySelector('.directorist-selected-icon')
						.setAttribute(
							'class',
							`directorist-selected-icon ${self.value}`
						);
			...
			document
				.querySelector('.icon-picker__close')
				.addEventListener('click', closeModal);
			document.body.addEventListener('click', (e) => {
				if (
					!e.target.closest('.icon-picker__inner') &&
					!e.target.closest('.icon-picker-selector') &&
					!e.target.closest('.icons-group-icons')
				) {
					closeModal();
				}
			});
			...
			/* Default icons pack */
			let iconFilter = document.querySelector(
		...
		findIconGroupByIconType: function (type) {
			...
			for (const iconGroup in this.icons) {
				for (const iconType in this.icons[iconGroup].iconTypes) {
					if (
						type !== this.icons[iconGroup].iconTypes[iconType].key
					) {
						continue;
					}
		...
		getFullIcon(iconKey, iconGroupKey, iconType) {
			let prefix = '';
			if (
				typeof this.icons[iconGroupKey].iconTypes[iconType] !==
				'undefined'
			) {
