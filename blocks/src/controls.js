...
export const TypesControl = withSelect( ( select ) => {
	...
	if ( props.items ) {
		choices = (
			props.shouldRender
				? sortItemsBySelected( props.items, props.selected, 'slug' )
				: props.items
		).map( ( item ) => (
			<li key={ item.id }>
				<CheckboxControl
					...
					onChange={ ( updated ) => {
						const values = updated
							? [ ...props.selected, item.slug ]
							: without( props.selected, item.slug );
			...
			defaultDirChoices = (
				props.selected.length > 0
					? props.items.filter( ( item ) =>
							props.selected.includes( item.slug )
					  )
					: props.items
			).map( ( item ) => {
				return {
	...
	return (
		<Fragment>
			...
			>
				<ul className="editor-post-taxonomies__hierarchical-terms-list">
					{ choices }
				</ul>
			</BaseControl>
...
class TermsMultiSelectComponent extends Component {
	...
	static getDerivedStateFromProps( props, state ) {
		if ( ! isEmpty( props.options ) || ! isEmpty( state.options ) ) {
			return {
				options: uniqBy(
					state.options.concat( props.options ),
					'value'
				),
				isLoading: false,
			};
	...
	render() {
		...
		return (
			<TokenMultiSelectControl
				...
				onInputChange={ ( term ) => {
					apiFetch( {
						path: `wp/v2/${ this.props.taxonomy }?per_page=10&search=${ term }`,
					} ).then( ( terms ) => {
...
export const TagsControl = ( props ) => (
	<TermsMultiSelectControl
		{ ...props }
		taxonomy={ TAG_TAX }
		label={ __( 'Select Tags', 'directorist' ) }
	/>
);
...
export const LocationControl = ( props ) => (
	<TermsMultiSelectControl
		...
		label={ __( 'Select Locations', 'directorist' ) }
	/>
);
	...
	render() {
		...
		return (
			<TokenMultiSelectControl
				...
				onInputChange={ ( term ) => {
					apiFetch( {
						path: `wp/v2/${ this.props.postType }?per_page=10&search=${ term }`,
					} ).then( ( posts ) => {
...
export const ListingControl = ( props ) => (
	<PostsMultiSelectControl
		{ ...props }
		postType={ POST_TYPE }
		label={ __( 'Select Listings', 'directorist' ) }
	/>
);
