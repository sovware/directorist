...
- Field badge condition metadata reads raw directory builder fields without the `listing_submission_form` context so Pricing Plans or other add-listing gating does not hide condition choices in settings. Runtime conditions read listing post meta by field key, first with the `_field_key` prefix and then unprefixed. The value can be adjusted with `directorist_badge_rule_field_value`.
- Pricing badge conditions read the legacy listing plan meta key `_fm_plans`, the v4 Pricing Plans `_plan_id` key when `directorist_plan_key()` is available, and the listing's active v4 package when `directorist_user_package_repository()` is available. They can be adjusted with `directorist_badge_rule_pricing_plan_meta_keys` and `directorist_badge_rule_pricing_plan_ids`.
...
The other core default icons are `la la-fire` for Popular and `la la-star-o` for Featured. Custom badge defaults use `la la-certificate` unless the admin chooses another icon. The saved icon feeds the builder widget icon class and text badge frontend/admin preview output when present. Badge colors and icon-only badge type do not change the builder chip UI. Removing a text badge icon saves an intentionally empty `icon` with `iconEdited: true`, and the badge renders label-only. The UI must allow `conditions: []`; manual Add condition is the normal user action that creates a condition row, while Reset defaults recreates the default New, Popular, and Featured condition rows. The reset confirmation must not auto-save; it only applies previewable draft changes until the admin clicks Save changes.

Supported core General condition keys in PHP and UI are `age_days`, `view_count`, `average_rating`, `review_count`, and `is_featured`; extra General options must register both metadata through `directorist_badge_rule_general_condition_options` and runtime values through `directorist_badge_rule_general_condition_value`. Unsupported saved General rows, such as old `listing_status` data, must not be converted into another supported condition. Field condition keys are discovered from supported listing submission fields and read listing meta. Pricing plan condition keys are `has_plan` and `plan_id`. The UI supports `all` and `any` matching and add/remove condition rows for core and custom badge cards. Legacy-compatible core General condition rows also mirror back to `new_listing_day`, `listing_popular_by`, `views_for_popular`, and `average_review_for_popular`; Popular reset keeps `listing_popular_by` mirrored to `view_count` while the second `average_rating` row mirrors `average_review_for_popular`.

Field source metadata supports text-like fields, numeric fields, exact date/time/color fields, select/radio/checkbox fields, switch fields, taxonomy fields, and presence-style fields such as image, video, file, map, privacy, and button fields. Select-style options must tolerate both Directorist option shapes: `value`/`label` and `option_value`/`option_label`. Taxonomy, checkbox, map, and button rows use option-membership operators such as `is`, `is not`, `contains`, `does not contain`, `is empty`, and `is not empty`; they must not fall through to numeric operators.

Badge condition operator controls are grouped into five dropdown option sets, even though every operator control renders as a select in the UI: number comparison (`greater or equal`, `is greater than`, `less or equal`, `is less than`, `equal to`, `not equal`), text comparison (`equal to`, `not equal`, `contains`, `does not contain`, `is empty`, `is not empty`), exact select (`is`, `is not`), option membership (`is`, `is not`, `contains`, `does not contain`, `is empty`, `is not empty`), and exact input (`is`, `is not`, `is empty`, `is not empty`). Keep saved operator values as raw tokens such as `>=`, `<=`, `=`, and `is_not`; only the admin-facing labels should be human-readable.

Pricing source metadata lists published legacy core pricing plans from `atbdp_pricing_plans`, v4 Pricing Plans rows from `directorist_plans`, and WooCommerce listing pricing products where available. Treat the runtime plan-ID resolver as extension-sensitive: core reads `_fm_plans`, v4 `_plan_id`, and the active v4 package by default, and extensions should use the provided filters instead of hard-coding extension-specific storage outside the adapter.
