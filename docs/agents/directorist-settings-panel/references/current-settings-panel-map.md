...
- The temporary `Needs Design` fallback remains only as a compatibility safety net for future extension-injected or newly added unmapped fields. Core Search, User, All Listings, and Category/Location settings have been given permanent homes in the redesigned navigation: `Search` is a top-level menu with `Search form` and `Search results`; `Users & accounts` is a top-level menu with `Registration`, `Login`, `Dashboard`, and `Authors`; `Directory` owns `Listings page` and `Categories & locations`. Do not reintroduce these core groups under `Needs Design`.
