...
- Future `Needs Design` work should start with research, not implementation. Inventory every menu/section/option currently under `Needs Design`, map each option to the closest existing redesigned pattern, and decide whether it should align into an existing menu/submenu or remain separate. Present the menu-alignment and design-pattern decisions step by step for user confirmation before making code changes.
