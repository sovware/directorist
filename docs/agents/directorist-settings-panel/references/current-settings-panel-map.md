...
- Sidebar search functionality is intentionally deferred until the current visual redesign is complete. Future search work should first inspect the reference interaction: click/focus opens a different quick-result UI, and typing should show matching results plus new recent-search entries. Plan the data/state shape before implementation, including where recent searches live, how search result clicks update that history, empty-state behavior, keyboard/focus handling, and preserving existing `jumpToSearchResult` navigation.
