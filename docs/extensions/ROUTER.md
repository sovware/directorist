...
For intake and reproduction follow [the focused issue workflow](WORKFLOW.md). Bengali/Banglish phrase aliases are in [intake-aliases.json](intake-aliases.json); they are intentionally finite hints, not general language understanding. Unsupported wording can remain unresolved, so an agent should translate/paraphrase the symptom or inspect active source instead of forcing a match.
