...
class Helper {
    ...
    /**
     * Returns the complete listing card image HTML for a given listing.
     *
     * Consolidates the thumb-card template and thumbnail rendering into a
     * single static call — no Directorist_Listings instantiation required.
     * Image attachment data is fetched once and reused for both the front
     * image and the optional blur-background layer.
     *
     * @param int $listing_id
     * @return string
     */
    public static function get_listing_card_image( int $listing_id ): string {
        $image_size    = get_directorist_option( 'way_to_show_preview', 'cover' );
        $is_blur       = 'blur' === get_directorist_option( 'prv_background_type', 'blur' );
        $by_ratio      = 'px' !== get_directorist_option( 'prv_container_size_by', 'px' );
        $ratio_width   = (int) get_directorist_option( 'crop_width', 360 );
        $ratio_height  = (int) get_directorist_option( 'crop_height', 300 );
        $image_quality = get_directorist_option( 'preview_image_quality', 'directorist_preview' );
        $disable_link  = (bool) get_directorist_option( 'disable_single_listing', false );

        // Container style
        $style = $by_ratio
            ? 'padding-top:' . ( $ratio_height / $ratio_width * 100 ) . '%'
            : "height:{$ratio_height}px";
        if ( 'full' !== $image_size && ! $is_blur ) {
            $style .= ';background-color:' . get_directorist_option( 'prv_background_color', '#fff' );
        }

        // Image data — fetched once, shared for front and blur-back
        $listing_type  = directorist_get_listing_directory( $listing_id );
        $default_image = self::default_preview_image_src( $listing_type );
        $thumbnail_ids = array_filter(
            array_merge(
                (array) directorist_get_listing_preview_image( $listing_id ),
                (array) directorist_get_listing_gallery_images( $listing_id )
            ),
            'is_numeric'
        );

        $permalink  = esc_url( apply_filters( 'directorist_archive_single_listing_url', get_permalink( $listing_id ), $listing_id, 'thumbnail' ) );
        $link_open  = "<a href='{$permalink}'><figure>";
        $link_close = '</figure></a>';

        $front_html = self::listing_thumbnail_img( $thumbnail_ids, $default_image, $listing_id, 'directorist-thumnail-card-front-img', $image_quality, $disable_link, $link_open, $link_close );
        $front_wrap = "<div class='directorist-thumnail-card-front-wrap'>{$front_html}</div>";

        $blur_bg = '';
        if ( $is_blur ) {
            $back_html = self::listing_thumbnail_img( $thumbnail_ids, $default_image, $listing_id, 'directorist-thumnail-card-back-img', $image_quality, $disable_link, $link_open, $link_close );
            $blur_bg   = "<div class='directorist-thumnail-card-back-wrap'>{$back_html}</div>";
        }

        $wrap_classes = [ 'contain' => 'directorist-card-contain', 'full' => 'directorist-card-full', 'cover' => 'directorist-card-cover' ];
        $wrap_class   = $wrap_classes[ $image_size ] ?? 'directorist-card-cover';

        return "<div class='directorist-thumnail-card {$wrap_class}' style='{$style}'>{$blur_bg}{$front_wrap}</div>";
    }

    private static function listing_thumbnail_img( array $thumbnail_ids, string $default_image, int $listing_id, string $class, string $image_quality, bool $disable_link, string $link_open, string $link_close ): string {
        if ( empty( $thumbnail_ids ) ) {
            $img = "<img src='{$default_image}' alt='" . esc_attr( get_the_title( $listing_id ) ) . "' class='{$class}' />";
            return $disable_link ? $img : "{$link_open}{$img}{$link_close}";
        }

        if ( 1 === count( $thumbnail_ids ) ) {
            $id  = reset( $thumbnail_ids );
            $alt = get_post_meta( $id, '_wp_attachment_image_alt', true ) ?: esc_html( get_the_title( $id ) );
            $img = "<img src='" . esc_url( atbdp_get_image_source( $id, $image_quality ) ) . "' alt='" . esc_attr( $alt ) . "' class='{$class}' />";
            return $disable_link ? $img : "{$link_open}{$img}{$link_close}";
        }
        ...
        foreach ( $thumbnail_ids as $id ) {
            $alt    = get_post_meta( $id, '_wp_attachment_image_alt', true ) ?: esc_html( get_the_title( $id ) );
            $img    = "<img src='" . esc_url( atbdp_get_image_source( $id, $image_quality ) ) . "' alt='" . esc_attr( $alt ) . "' class='{$class}' />";
            $slide  = $disable_link ? $img : "{$link_open}{$img}{$link_close}";
            $slides .= "<div class='swiper-slide'>{$slide}</div>";
        }

        return "<div class='directorist-swiper directorist-swiper-listing' data-sw-items='1' data-sw-margin='2' data-sw-loop='true' data-sw-perslide='1' data-sw-speed='500' data-sw-autoplay='false' data-sw-responsive='{}'>
