	...
	class ATBDP_Add_Listing {
		...
		public function atbdp_submit_listing() {
			try {
				...
				$error                     = array();
				$meta_data                 = array();
				$new_l_status           = get_term_meta( $directory_id, 'new_listing_status', true );
				$edit_l_status          = get_term_meta( $directory_id, 'edit_listing_status', true );
				...
				if ( ! empty( $posted_tags ) && is_array( $posted_tags ) ) {
					$posted_tags = array_map( 'directorist_sanitize_term_item', $posted_tags );
				}

				if ( ! empty( $posted_locations ) && is_array( $posted_locations ) ) {
					$posted_locations = array_map( 'directorist_sanitize_term_item', $posted_locations );
				}

				if ( ! empty( $posted_categories ) && is_array( $posted_categories ) ) {
					$posted_categories = array_map( 'directorist_sanitize_term_item', $posted_categories );
				}

				/**
				 * Process form fields.
				 */
				$submission_form_fields = directorist_get_listing_submission_form_fields( $directory_id );
				...
				foreach ( $submission_form_fields as $field_internal_key => $form_field ) {
					...
					if ( $field->is_admin_only() && ! current_user_can( get_post_type_object( ATBDP_POST_TYPE )->cap->edit_others_posts ) ) {
						continue;
					}

					switch ( $field->get_internal_key() ) {
						// case 'location':
						// 	$this->process_locations( $field, $posted_data, $error );
						// 	break;
						
						// case 'category':
						// 	$this->process_categories( $field, $posted_data, $error );
						// 	break;

						// case 'tag':
						// 	$this->process_tags( $field, $posted_data, $error );
						// 	break;
					}
					...
					// if ( ! $field->is_preset() ) {
					// 	$_meta_data['_' . $field->get_key() ] = $field->sanitize( $posted_data );
					// }
								...
								foreach ( $posted_locations as $location ) {
									...
									if ( $is_location_insert_allowed ) {
										...
										if ( is_wp_error( $location_added ) ) {
											...
											update_term_meta( $location_added['term_id'], '_directory_type', array( $directory_id ) );
										}
									}
								}
								...
								foreach ( $posted_categories as $category ) {
									...
									if ( $is_category_insert_allowed ) {
										...
										if ( is_wp_error( $category_added ) ) {
											...
											update_term_meta( $category_added['term_id'], '_directory_type', array( $directory_id ) );
										}
									}
								}
