...
class Enqueue_Assets {
	 ...
	 * @return void
	 */
	public static function enqueue_public_scripts( $fource_enqueue = false ) {
		// Load Assets
		self::load_assets();
		...
		self::enqueue_css_scripts_by_group( [ 'group' => 'public', 'fource_enqueue' => $fource_enqueue ] );
		wp_add_inline_style( 'directorist-inline-style', self::dynamic_style() );

		// JS
		self::register_js_scripts();
		self::enqueue_js_scripts_by_group( [ 'group' => 'public', 'fource_enqueue' => $fource_enqueue ] );
	}
	 ...
	 * @return void
	 */
	public static function enqueue_admin_scripts( $page = '', $fource_enqueue = false ) {
		// Load Assets
		self::load_assets();
	...
	public static function add_vendor_css_scripts() {
		...
		// Global
		// ================================
		$scripts['directorist-unicons'] = array_merge( $default, [
			'group' => 'global',
		...
		$scripts['directorist-select2'] = array_merge( $default, [
			'group'     => 'global',
			'file_name' => 'select2',
			'base_path' => DIRECTORIST_VENDOR_CSS,
			'has_unmin' => false,
		...
		$scripts['directorist-font-awesome'] = array_merge( $default, [
			'group'     => 'global',
			'file_name' => 'font-awesome',
			'has_unmin' => false,
			'has_rtl'   => false,
		]);

		$scripts['directorist-line-awesome'] = array_merge( $default, [
			'group'     => 'global',
			'file_name' => 'line-awesome',
			'has_unmin' => false,
			'has_rtl'   => false,
		]);
		...
		// Public
		// ================================
		$scripts['directorist-slick'] = array_merge( $default, [
			'group'        => 'public',
			'file_name'    => 'slick',
			'has_unmin'    => false,
			'has_rtl'      => false,
		...
		$scripts['directorist-sweetalert'] = array_merge( $default, [
			'group'     => 'public',
			'file_name' => 'sweetalert',
			'has_unmin' => false,
			// 'load_on_init' => is_singular( ATBDP_POST_TYPE ),
		]);

		$scripts['directorist-ez-media-uploader'] = array_merge( $default, [
			'group'     => 'public',
	...
	public static function add_admin_js_scripts() {
		...
		$scripts['directorist-custom-field'] = array_merge( $default, [
			'file_name'     => 'admin-custom-field',
			'load_on_init'  => false,
		]);

		$scripts['directorist-extension-update'] = array_merge( $default, [
			'file_name'     => 'admin-extension-update',
			'load_on_init'  => false,
		]);
	...
	public static function add_global_js_scripts() {
		...
		$scripts['directorist-range-slider'] = array_merge( $default, [
			'file_name' => 'global-range-slider',
			// 'load_on_init'        => false,
		]);

		$scripts['directorist-add-listing'] = array_merge( $default, [
		...
		$scripts['directorist-add-listing-openstreet-map-custom-script'] = array_merge( $default, [
			'file_name'    => 'global-add-listing-openstreet-map-custom-script',
			'enable'       => Script_Helper::is_enable_map( 'openstreet' ),
			'load_on_init' => false,
		]);

		$scripts['directorist-add-listing-gmap-custom-script'] = array_merge( $default, [
		...
		$scripts['directorist-pure-select'] = array_merge( $default, [
	...
	public static function get_script_file_name( array $args = [] ) {
		$default = [ 
			...
			'has_unmin'           => true,
			'has_rtl'             => true,
			'rtl_prefix_position' => 'after_min'
		];
		$args    = array_merge( $default, $args );

		$file_name = ( ! empty( $args['file_name'] ) ) ? $args['file_name'] : '';
		$has_unmin = ( ! empty( $args['has_unmin'] ) ) ? $args['has_unmin'] : false;
		...
		$is_rtl   = is_rtl();
		$load_rtl = ( $has_rtl && $is_rtl ) ? true : false;


		if ( 'before_min' === $args['rtl_prefix_position'] && $load_rtl ) {
			$file_name = "{$file_name}.rtl";
		}

		if ( ! $args['has_unmin'] || ( $has_min && $load_min ) ) {
			$file_name = "{$file_name}.min";
		}

		if ( 'after_min' === $args['rtl_prefix_position'] && $load_rtl ) {
			$file_name = "{$file_name}.rtl";
		}
