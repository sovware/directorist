...
if ( ! class_exists( 'ATBDP_Shortcode' ) ):
    class ATBDP_Shortcode {
        ...
        public function add_listing( $atts, $content = null, $sc_name ) {
            ...
            if ( atbdp_logged_in_user() || $guest ) {
                ...
                $on_add_listing_page    = (  ( strpos( $current_url, '/edit/' ) !== false ) && ( $pagenow = 'at_biz_dir' ) ) ? true : false;
                $monetization_is_active = ( is_fee_manager_active() && ! selected_plan_id() ) ? true : false;
                $has_pricing_plan       = ( class_exists( 'ATBDP_Pricing_Plans' ) ) ? true : false;

                if ( $monetization_is_active && ! $on_add_listing_page && $has_pricing_plan ) {
    ...
    public function all_categories( $atts ) {
        ...
        if ( 'yes' == $logged_in_user_only && ! atbdp_logged_in_user() ) {
            return $this->guard( array( 'type' => 'auth' ) );
        }
    ...
    public function all_listing( $atts ) {
        $listings_model = new Directorist_All_Listings( array( 'atts' => $atts ) );
    ...
    public function atbdp_tag( $atts ) {
        ...
        if ( '' != $term_slug ) {
            ...
            if ( 'listings_with_map' == $view ) {
                ...
                if ( $path ) {include $path;} else {include BDM_TEMPLATES_DIR . '/map-view.php';}
    ...
    public function custom_user_login() {
        ...
        $path = atbdp_get_theme_file( '/directorist/shortcodes/auth/login.php' );
        if ( $path ) {
            include $path;
        } else {
            include ATBDP_TEMPLATES_DIR . 'public-templates/shortcodes/auth/login.php';
        }
    ...
    public function directorist_listing_map() {
        ...
        if ( is_singular( ATBDP_POST_TYPE ) ) {
            ...
            $reviews       = (  ( $reviews_count > 1 ) || ( 0 === $reviews_count ) ) ? __( ' Reviews', 'directorist' ) : __( ' Review', 'directorist' );
            $review_info   = '';
            $review_info   = '';
    ...
    public function directorist_related_listings() {
        ...
        if ( is_singular( ATBDP_POST_TYPE ) ) {
            ...
            if ( empty( $enable_rel_listing ) ) {
                return;
            }
            // vail if related listing is not enabled
    ...
    public function search_listing( $atts, $content = null ) {
        ...
        $search_subtitle            = get_directorist_option( 'search_subtitle', __( 'Find the best match of your interest', 'directorist' ) );
    ...
    public function search_result( $atts ) {
        ...
        if ( class_exists( 'Post_Your_Need' ) && ( 'need' === $listing_type ) ) {
    ...
    public function user_registration() {
        ...
        if ( ! atbdp_logged_in_user() ) {
            ...
            $path = atbdp_get_theme_file( '/directorist/shortcodes/auth/registration.php' );
            if ( $path ) {
                include $path;
            } else {
                include ATBDP_TEMPLATES_DIR . 'public-templates/shortcodes/auth/registration.php';
            }
        } else {
