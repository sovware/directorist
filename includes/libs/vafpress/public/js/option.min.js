;(function($) {
	...
	function arrayToObject( arr ) {
		...
		for ( var i = 0; i < arr.length; i++) {
			var obj_item = obj[ arr[i].name ];
			var arr_name = arr[i].name;
			var arr_value = arr[i].value;
			
			if ( obj.hasOwnProperty( arr_name ) ) {
				if ( Array.isArray(obj_item) && obj_item.length ) {
					obj[ arr_name ].push(arr_value);
				} else {
					var old_data = obj[arr_name];
					obj[ arr_name ] = [old_data];
					obj[ arr_name ].push(arr_value);
				}
			} else {
	...
	var old_form_data = $('#vp-option-form').serializeArray();
	...
	$('.vp-js-option-form').bind('submit', function(e) {
		...
		var old_form_data_obj = arrayToObject( old_form_data );
		var option_obj = arrayToObject( option );
		...
		$("#vp-option-form :input").each(function(index, elm) {
			...
			if ( 'checkbox' === input_type && !(option_obj.hasOwnProperty( input_name )) ) {
				var obj_item = option_obj[ input_name ];
				var arr_name = input_name;
				var arr_value = value;
				
				if ( option_obj.hasOwnProperty( arr_name ) ) {
				if ( Array.isArray(obj_item) && obj_item.length ) {
					option_obj[ arr_name ].push(arr_value);
				} else {
					var old_data = option_obj[arr_name];
					option_obj[ arr_name ] = [old_data];
					option_obj[ arr_name ].push(arr_value);
					}
				} else {
					option_obj[arr_name] = value;
				}
			}
		});
		
		var updated_data = [];
		for ( var option_item in option_obj ) {
			var old_value = old_form_data_obj[option_item];
			old_value = ( typeof old_value === 'undefined' ) ? '' : old_value;
			var _old_value = ( Array.isArray( old_value ) ) ? JSON.stringify(old_value) : old_value;
			
			var new_value = option_obj[option_item];
		...
		$.post(ajaxurl, data, function(response) {
			...
			{
				$save_status.addClass('success');
				old_form_data = $('#vp-option-form').serializeArray();
			}
			else
			{
