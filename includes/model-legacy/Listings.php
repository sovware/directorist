...
class Directorist_Listings {
	...
	public function render_shortcode() {
		...
		ATBDP()->enquirer->search_listing_scripts_styles( ['directory_type_id' => $this->current_listing_type] );
		...
		Helper::get_template( $template_file, array('listings' => $this), 'listings_archive' );
	...
	public function loop_template( $loop = 'grid' ) {
		while ($this->query->have_posts()) {
			$this->query->the_post();
			$this->set_loop_data();
			Helper::get_template( "listings-archive/loop/$loop", array('listings' => $this) );
		}
		wp_reset_postdata();
	}
		...
		if ( ! empty( $listings->ids ) ) :
			...
			foreach ( $listings->ids as $listings_id ) :
				...
				if (!empty($opt['display_map_info']) && (!empty($opt['display_image_map']) || !empty($opt['display_title_map']) || $opt['display_address_map']) || !empty($opt['display_direction_map'])) {
					Helper::get_template( 'listings-archive/loop/openstreet-map', $opt );
				}
	...
	public function load_google_map() {
			...
			if ( ! empty( $listings->ids ) ) :
				...
				foreach ( $listings->ids as $listings_id ) :
					...
					if ( ! empty( $ls_data['manual_lat'] ) && ! empty( $ls_data['manual_lng'] ) ) {
						$opt['ls_data'] = $ls_data;
						Helper::get_template( 'listings-archive/loop/google-map', $opt );
					}
		...
		public function loop_thumb_card_template() {
			Helper::get_template( 'listings-archive/loop/thumb-card', array('listings' => $this) );
		}
		...
		public function sortby_dropdown_template() {
			$html = Helper::get_template_contents( 'listings-archive/sortby-dropdown', array('listings' => $this) );
			echo apply_filters('atbdp_listings_header_sort_by_button', $html);
		}

		public function viewas_dropdown_template() {
			$html = Helper::get_template_contents( 'listings-archive/viewas-dropdown', array('listings' => $this) );
		...
		public function advanced_search_form_template() {
			...
			Helper::get_template( 'listings-archive/advanced-search-form', $args );
		}
		...
		public static function archive_type($listings) {
			...
			if ( $count > 1 && ! empty( $enable_multi_directory ) ) {
				Helper::get_template( 'listings-archive/listing-types', array('listings' => $listings) );
			}
		}
		...
		public static function archive_header($listings) {
			...
			Helper::get_template( 'listings-archive/listings-header', array('listings' => $listings) );
		}
