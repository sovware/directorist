...
class License {
        ...
        <div class="wrap appsero-license-settings-wrapper">
            ...
            <div class="appsero-license-settings appsero-license-section">
                ...
                <div class="appsero-license-details">
                    <p>
                        <?php esc_html( sprintf( $this->client->_trans( 'Activate <strong>%s</strong> by your license key to get professional support and automatic update from your WordPress dashboard.' ), $this->client->name ) ); ?>
                    </p>
                    <form method="post" action="<?php $this->formActionUrl(); ?>" novalidate="novalidate" spellcheck="false">
                        ...
                        <div class="license-input-fields">
                            ...
                            <button type="submit" name="submit" class="<?php echo 'deactive' == $action ? 'deactive-button' : ''; ?>">
                                <?php echo $action == 'active' ? esc_attr( $this->client->_trans( 'Activate License' ) ) : esc_attr( $this->client->_trans( 'Deactivate License' ) ); ?>
                            </button>
                        </div>
                    </form>
            ...
            <div class="single-license-info">
                ...
                <?php } else { ?>
                    <p class="<?php echo $license['remaining'] ? '' : 'occupied'; ?>">
                        <?php esc_html( sprintf( $this->client->_trans( '%1$d out of %2$d' ), $license['remaining'], $license['activation_limit'] ) ); ?>
                    </p>
                <?php } ?>
            </div>
            <div class="single-license-info">
                ...
                <?php
                    if ( false !== $license['expiry_days'] ) {
                        ...
                        echo '<p class="' . esc_attr( $occupied ) . '">' . esc_attr( $license['expiry_days'] ) . ' days</p>';
                    } else {
                        echo '<p>' . esc_attr( $this->client->_trans( 'Never' ) ) . '</p>';
                    } ?>
            </div>
        </div>
        <?php
    ...
    private function formActionUrl() {
        $url = add_query_arg(
            $_GET,
            admin_url( basename( $_SERVER['SCRIPT_NAME'] ) ) // @codingStandardsIgnoreLine.
        );
