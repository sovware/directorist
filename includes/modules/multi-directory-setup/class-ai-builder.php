...
Please regenerate the fields and must include the pinned fields list while maintaining relevance to the selected keywords. Return all fields in JSON format with only the following keys for each field: "type", "label", "group", and, if applicable, "options".
USER_PROMPT;
	...
	protected static function prepare_fields( $fields ) {
		...
		foreach ( $fields as $field ) {
			...
			if ( isset( $preset_fields_map[ $field['type'] ] ) ) {
				$field_name = $preset_fields_map[ $field['type'] ];
