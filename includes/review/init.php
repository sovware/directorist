...
class Bootstrap {
    ...
    public static function setup_hooks() {
        ...
        add_filter( 'register_post_type_args', [ __CLASS__, 'add_comment_support' ], 10, 2 );
        add_filter( 'map_meta_cap', [ __CLASS__, 'map_meta_cap_for_review_author' ], 10, 4 );
        add_filter( 'atbdp_login_redirection_page_url', [ __CLASS__, 'setup_login_redirect' ] );
        add_filter( 'safe_style_css', [ __CLASS__, 'add_safe_style_css' ] );
        add_filter( 'option_comment_registration', [ __CLASS__, 'reset_option_comment_registration' ] );
    }
    ...
    public static function setup_login_redirect( $redirect ) {
        if ( ! empty( $_GET['redirect'] ) ) {
            ...
            return wp_validate_redirect( wp_unslash( $_GET['redirect'] ), home_url( '/' ) ) . $scope;
        }
    ...
    public static function override_comments_pagination( $wp_query ) {
        if ( ! is_admin() && directorist_is_review_enabled() && $wp_query->is_single && $wp_query->get( 'post_type' ) === ATBDP_POST_TYPE ) {
            ...
            add_filter( 'option_thread_comments_depth', [ __CLASS__, 'override_comment_depth' ] );
            add_filter( 'option_comments_per_page', 'directorist_get_review_per_page' );
            add_filter( 'option_default_comments_page', [ __CLASS__, 'override_default_comments_page_option' ] );
            add_filter( 'option_comment_order', [ __CLASS__, 'override_comment_order_option' ] );
