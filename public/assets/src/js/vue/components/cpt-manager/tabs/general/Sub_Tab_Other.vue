            ...
            <div class="cptm-form-fields">
                <template v-for="( field, field_key ) in section.fields">
                    <component 
                        ...
                        :key="field_key"
                        v-bind="field"
                        @update="updateOtherSectionData( section_key, field_key, $event )">
                    </component>
                </template>
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...mapState({
            general_sections: state => state.settings.general.submenu.other.sections
        }),
    },
    data() {
        return {
    ...
    methods: {
        updateOtherSectionData( section_key, field_key, value ) {
            console.log(  { section_key, field_key, value } );
            this.$store.commit( 'updateOtherSectionData', { section_key, field_key, value } );
        }
    }
}
</script>