    ...
    <div class="" v-for="( section, section_index ) in general_sections" :key="section_index">
        ...
        <div class="cptm-form-fields">
            <template v-for="( field, field_index ) in section.fields">
                <component :is="fields[ field.type ]" :key="field_index" v-bind="field" v-model="field.value">
                </component>
            </template>
        </div>
    </div>
...
export default {
    ...
    computed: {
        ...
        general_sections() {
            return this.objectToSectionArray(this.$store.state.settings.general.submenu.packages.sections);
        }
    },
    data() {
        return {
