				...
				<div class="add_listing_title atbd_success_mesage">
					...
					if ( ! empty( $_GET['registration_status'] ) && true == $_GET['registration_status'] ) {
						...
						<?php } else { ?>
							...
							<p style="padding: 20px" class="alert-success"><span class="fa fa-check"></span><?php _e(' Registration completed. Please check your email for confirmation.', 'directorist'); ?>
				...
				<div class="directory_register_form_wrap">
					<form action="<?php the_permalink(); ?>" method="post">
						<div class="directorist-form-group directorist-mb-15">
							...
							<input id="username" class="directorist-form-element" type="text" name="username" value="<?php echo ( isset( $_REQUEST['username'] ) ? esc_attr( $_REQUEST['username'] ) : '' ); ?>">
						</div>
						<div class="directorist-form-group directorist-mb-15">
							<label for="email"><?php echo esc_html( $email ); ?> <strong class="directorist-form-required">*</strong></label>
							<input id="email" class="directorist-form-element" type="text" name="email" value="<?php echo ( isset( $_REQUEST['email'] ) ? esc_attr( $_REQUEST['email'] ) : '' ); ?>">
						</div>
						...
						<div class="directorist-form-group directorist-mb-15">
							...
							?></label>
							<input id="fname" class="directorist-form-element" type="text" name="fname" value="<?php echo ( isset( $_REQUEST['fname']) ? esc_attr( $_REQUEST['fname'] ) : '' ); ?>">
						</div>
						<?php } ?>
						...
						<div class="directorist-form-group directorist-mb-15">
							...
							?></label>
							<input class="directorist-form-element" id="lname" type="text" name="lname" value="<?php echo ( isset( $_REQUEST['lname']) ? esc_attr( $_REQUEST['lname'] ) : '' ); ?>">
						</div>
						<?php } ?>
						<?php if ( ! empty( $display_website ) ) {  ?>
							<div class="directorist-form-group directorist-mb-15">
								...
								?></label>
								<input id="website" class="directorist-form-element" type="text" name="website" value="<?php echo ( isset( $_REQUEST['website']) ? esc_url( $_REQUEST['website'] ) : '' ); ?>">
							</div>
						<?php } ?>
						...
						<div class="directorist-form-group directorist-mb-15">
							...
							?></label>
							<textarea id="bio" class="directorist-form-element" name="bio" rows="10"><?php echo ( isset( $_REQUEST['bio']) ? esc_textarea( $_REQUEST['bio'] ) : '' ); ?></textarea>
						</div>
						<?php } ?>
						<?php if ( ! empty( get_directorist_option( 'display_user_type' ) ) ) {
							...
							<div class="atbd_user_type_area directory_regi_btn directorist-radio directorist-radio-circle directorist-mb-15">
								<input id="author_type" type="radio" name="user_type" value="author" <?php echo $author_checked; ?>><label for="author_type" class="directorist-radio__label"><?php esc_html_e( 'I am an author', 'directorist' ); ?>
							</div>
							<?php } ?>
							<?php if ( empty( $user_type ) || 'general' == $user_type ) { ?>
							<div class="atbd_user_type_area directory_regi_btn directorist-radio directorist-radio-circle directorist-mb-15">
								<input id="general_type" type="radio" name="user_type" value="general" <?php echo $general_checked; ?>><label for="general_type" class="directorist-radio__label"><?php esc_html_e( 'I am a user', 'directorist' ); ?>
							</div>
							<?php } ?>
						<?php } ?>
