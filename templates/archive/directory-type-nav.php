	...
	<div class="directorist-type-nav">
		...
		<ul class="directorist-type-nav__list">
			...
			<?php if ( ! empty( $all_types ) ) : ?>
				...
				<li class="list-inline-item <?php echo ( $listings->get_current_listing_type() === 'all' || 'all' === $current_directory_type ) ? 'current' : ''; ?>">
			...
			<?php foreach ( $listings->get_listing_types() as $id => $value ) : ?>
				...
				<li class="<?php echo ( ( $listings->get_current_listing_type() === $value['term']->term_id && 'all' !== $current_directory_type ) ? 'current': '' ); ?>">
