...
<div class="directorist-listing-category">
	<?php if ( ! empty( $listings->loop['cats'] ) ) {
		...
		?>
		<a href="<?php echo esc_url( $term_link ); ?>"><?php echo wp_kses_post( $term_icon ) . esc_html( $listings->loop['cats'][0]->name); ?></a>
		<?php
			...
			<div class="directorist-listing-category__popup">
				...
				<div class="directorist-listing-category__popup__content">
					<?php foreach ( array_slice( $listings->loop['cats'], 1 ) as $cat ) {
						...
						echo "<a href='" . esc_url( $term_link ) . "'> " . wp_kses_post( $term_label ) . "</a>";
					} ?>
				</div>
