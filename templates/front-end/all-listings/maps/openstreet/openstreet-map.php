...
<script>
    var addressPoints = [
        ...
        foreach ( $all_listings->ids as $listings_id ) :
            ...
            $cat_icon  = !empty($cats) ? get_cat_icon($cats[0]->term_id) : '';
...
<?php foreach ( $marker_icons as $icon ) : ?>
    <input type="hidden" value="<?php echo !empty($icon['cats']) ? $icon['cat_icon'] : 'fa fa-map-marker';?>" class="openstreet_icon">
<?php endforeach; ?>
