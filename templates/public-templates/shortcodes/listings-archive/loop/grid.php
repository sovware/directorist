				...
				<div class="atbd_listing_image">
					...
					if ( $show_preview_image ) {
						...
						if (!$single_listing_is_disabled) { ?>
							<a href="<?php echo $listings_link; ?>" <?php echo $listings_link_attr; ?>><?php the_thumbnail_card(); ?></a>
							<?php
						}
						else {
							the_thumbnail_card();
						...
						<div class="atbd_author">
							<a href="<?php echo $author_link; ?>" aria-label="<?php echo $author_full_name; ?>" class="<?php echo $author_link_class; ?>">
								<?php if (!empty($u_pro_pic)) { ?>
									<img src="<?php echo esc_url($u_pro_pic[0]); ?>" alt="<?php _e( 'Author Image', 'directorist' );?>">
									<?php
								}
								else {
									echo $avatar_img;
			...
			<div class="atbd_listing_info">
					...
					<div class="atbd_content_upper">
						<?php if (!empty($display_title)) { ?>
							<h4 class="atbd_listing_title">
								<?php
								if ( ! $single_listing_is_disabled  ) {
									printf('<a href="%s"%s>%s</a>', $listings_link, $listings_link_attr, $listings_title);
								}
								else {
									echo $listings_title;
								}
				...
				if ( ! empty( $display_category ) || ! empty( $display_view_count ) ) {
					...
					<div class="atbd_listing_bottom_content">
							...
							<div class="atbd_content_left">
							    <div class="atbd_listing_category">
									<a href="<?php echo ATBDP_Permalink::atbdp_get_category_page($cats[0]) ?>">
										<span class="<?php echo atbdp_icon_type() ?>-tags"></span><?php echo $cats[0]->name; ?>
									</a>
					    ...
					    if ( ! empty( $display_category ) && empty( $cats ) ) { ?>
							<div class="atbd_content_left">
								<div class="atbd_listing_category">
						...
						if (!empty($display_view_count)) {
							...
							<ul class="atbd_content_right">
								<li class="atbd_count"><span class="<?php echo atbdp_icon_type() ?>-eye"></span><?php echo ( ! empty($post_view) ) ? $post_view : 0; ?></li>
							</ul>
							<?php
						}
