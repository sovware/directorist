<div class='atbdp-body atbdp-map embed-responsive embed-responsive-16by9 atbdp-margin-bottom'>
    ...
    <div class='media-left'>
        ...
        <a href='<?php echo get_the_permalink(); ?>'> <?php }?>
            <?php if ( ! empty( $ls_data['listing_prv_img'] ) ) {?>
            <img src='<?php echo esc_url( $ls_data['prv_image'] ); ?>' alt='<?php echo esc_html( stripslashes( get_the_title() ) ); ?>'> <?php }?>

            <?php if ( ! empty( $ls_data['listing_img'][0] ) && empty( $ls_data['listing_prv_img'] ) ) {?>
            <img src='<?php echo esc_url( $ls_data['gallery_img'] ); ?>' alt='<?php echo esc_html( stripslashes( get_the_title() ) ); ?>'> <?php }?>

            <?php if ( empty( $ls_data['listing_img'][0] ) && empty( $ls_data['listing_prv_img'] ) ) {?>
            <img src='<?php echo esc_url( $ls_data['default_image'] ); ?>' alt='<?php echo esc_html( stripslashes( get_the_title() ) ); ?>'> <?php }?>
    ...
    <?php if ( ! empty( $display_title_map ) ) {?>
        ...
        <?php } else {?>
            <h3 class='atbdp-no-margin'><?php get_the_title();?></h3>
        <?php }?>
        </div>
    <?php }
    ...
    if ( ! empty( $ls_data['address'] ) ) {
        ...
        <div class='osm-iw-get-location'>
            <a href='http://www.google.com/maps?daddr=<?php echo $ls_data['manual_lat'] . ',' . $ls_data['manual_lng']; ?>' target='_blank'>
                <?php _e( 'Get Direction', 'directorist' )?>
            </a>
