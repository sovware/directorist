...
if ((!$hide_contact_info) && !empty($address || $phone ||$phone2 ||$fax || $email || $website || $zip || $social) && empty($disable_contact_info)) { ?>
	<div class="atbd_content_module atbd_contact_information_module">
		...
		<div class="atbdb_content_module_contents">
				...
				<ul>
					...
					if (isset($phone) && !is_empty_v($phone) && !empty($display_phone_field) && $plan_phone) { ?>
						<li>
							...
							<div class="atbd_info">
								<a href="tel:<?php ATBDP_Helper::sanitize_tel_attr( $phone ); ?>"><?php echo esc_html( $phone ); ?></a>
							</div>
						</li>
						<?php
					}
					...
					if (isset($phone2) && !is_empty_v($phone2) && !empty($display_phone2_field)) { ?>
						<li>
							...
							<div class="atbd_info"><a href="tel:<?php ATBDP_Helper::sanitize_tel_attr( $phone2 ); ?>"><?php echo esc_html( $phone2 ); ?></a>
							</div>
						</li>
						<?php
					}
					...
					if (isset($fax) && !is_empty_v($fax) && !empty($display_fax_field)) { ?>
						<li>
							...
							<div class="atbd_info"><a href="tel:<?php ATBDP_Helper::sanitize_tel_attr( $fax ); ?>"><?php echo esc_html( $fax ); ?></a>
							</div>
						</li>
						<?php
					}
			...
			<?php if (!empty($social) && is_array($social) && !empty($display_social_info_field) && $plan_social_networks) { ?>
