...
<div class="directorist-card__body">
  <div class="directorist-widget-listing">
    <?php
    foreach ( $related_listings->posts as $listing ) {

        $listing_id = $listing->ID;
        // get only one parent or high level term object
        $top_category = ATBDP()->taxonomy->get_one_high_level_term( $listing_id, ATBDP_CATEGORY );
        $listing_img = directorist_get_listing_gallery_images( $listing_id );
        $listing_prv_img = directorist_get_listing_preview_image( $listing_id );
        $review_rating = directorist_get_listing_rating( $listing_id );
        ...
        $listing_pricing = get_post_meta( $listing_id, '_atbd_listing_pricing', true );
        ...
        <div class="directorist-widget-listing__image">
          <?php
            ...
            if ( empty( $disable_single_listing ) ) {
                ?>
            <a href="<?php echo esc_url( get_post_permalink( $listing_id ) ); ?>">
                <?php
            }
            $type          = directorist_get_listing_directory( $listing_id );
        ...
        <div class="directorist-widget-listing__content">
            <h4 class="directorist-widget-listing__title">
              <?php
                if ( empty( $disable_single_listing ) ) {
                    ?>
                <a href="<?php echo esc_url( get_post_permalink( $listing_id ) ); ?>"><?php echo esc_html( $listing->post_title ); ?></a>
                    <?php
                } else {
